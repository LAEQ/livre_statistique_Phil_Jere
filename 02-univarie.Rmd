...
```{r figFormeDistr, fig.align='center', echo=FALSE, include=identical(knitr:::pandoc_to(), 'html'), auto_pdf = TRUE, fig.cap="Formes d'une distribution et les coefficients d'asymétrie et d'aplatissement",  out.width='90%'}
...
df <- data.frame(
  Test = c("Kolmogorov-Smirnov",
           ...
           "Jarque-Bera"),
  Interpretation =
          c("Plus sa valeur est proche de zéro, plus la distribution est normale.  L'avantage de ce test est qu'il peut être utilisé pour vérifier si une variable si la distribution de n'importe qu'elle loi (autre que la loi normale).",
           ...
           "Ce test est une modification du test de Cramer-von Mises (CVM). Il peut être aussi utilisé pour tester d'autres distributions (uniforme, log-normale, exponentielle, Weibull, distribution de pareto généralisée, logistique, ).",
...
knitr::kable(df,
  ...
  caption = "Les différents tests d'hypothèses pour la normalité",
  booktabs = TRUE, valign = 't', row.names = FALSE)
```

Dans le tableau ci-dessous sont reportées les valeurs des différents tests pour les cinq types de distribution générées à la figure \@ref(fig:CourbeNormale). Sans suprise, pour l'ensemble des tests $P>0,05$ pour la distribution normale.
...
df <- data.frame(
  ...
  Skewed_L = rValeMaurelli(50, mean=0, sigma=1, skew=1.4, kurt=3),
  Skewed_R = rValeMaurelli(50, mean=0, sigma=1, skew=-1.4, kurt=3),

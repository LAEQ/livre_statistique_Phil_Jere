 ...
 * Autres *packages* :
  - **Hmisc** et **Hmisc** pour les statistiques descriptives pondérées
...
Les données individuelles peuvent être **agrégées** à un niveau supérieur. Prenons le cas de microdonnées d'un recensement. Les informations disponibles pour chaque individu sont agrégées par territoire géographique (province, région économique, division de recensement, subdivision de recensement, région et agglomération de recensement, secteurs de recensement, aires de diffusion, etc.) en fonction du lieu d'habitation des individus. Des sommaires statistiques – basées sur la moyenne, la médiane, la somme ou la proportion de chacune des variables mesurées au niveau individuel (âge, sexe, situation de famille, revenu, etc.) – sont alors construit pour ces différents découpages géographiques ([Statistique Canada](https://www.statcan.gc.ca/fra/idd/trousse/section5#a4){target="_blank"}).

L'aggrégation n'est pas nécessairement géographique. En éductation, il est fréquent de travailler avec des données concernant les élèves, mais agrégées au niveau des écoles. La figure \@ref(fig:figunivarie1b) donne un exemple simple d'agrégation de données individuelles.
...
#### Autres types de transformation  {#sect02552}
...
### Mise en œuvre dans ![](images/Rlogo.png) {#sect0256}
...
```{r GcourbeNormale, fig.align='center', include=identical(knitr:::pandoc_to(), 'html'), auto_pdf = TRUE, fig.cap="Histogramme avec courbe normale",  out.width='65%'}
...
#### Transformer une variable dans ![](images/Rlogo.png) {#sect02563}
...
```{r GTranf, fig.align='center', include=identical(knitr:::pandoc_to(), 'html'), auto_pdf = TRUE, fig.cap="Histogramme des transformations",  out.width='75%'}
...
### Mise en œuvre dans ![](images/Rlogo.png) {#sect0262}
...
## Pour aller un plus loin : les statistiques descriptives pondérées {#sect027}
...
De nomnbreux *packages* sont disponibles pour calculer des statistiques pondérées, dont notamment `Weighted.Desc.Stat` et `Hmisc` utilisés dans la syntaxe ci-dessous.
...
# Calcul des paramètres de position et de position
# Moyenne
Hmisc::wtd.mean(x, w)

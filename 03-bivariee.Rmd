...
avec $Y_{i}$ et $X_{i}$ sont respectivement les valeurs des variables indépendante et dépendante pour l'observation $i$. $\beta_{0}$ est la constante (*intercept* en anglais), soit la valeur de variable $Y$ quand $X$ est égal à 0. $\beta_{1}$ est le coefficient de régression pour la variable *X*, soit la pente de la droite. Ce coefficient nous informe sur la relation entre les deux variables : s’il est positif, la relation est positive; s’il est négatif, la relation est négative, et proche de 0, la relation est nulle (la droite sera alors horizontale). Plus la valeur absolue de $\beta_{1}$ est élevée, plus la variable *Y* varie à chaque changement d’une unité de la variable *X*.

Considérons un exemple fictif de dix municipalités d'une région métropolitaine donnée pour lesquelles nous disposons de deux variables : le pourcentage d'actifs occupés se rendant au travail principalement à vélo au travail et la distance de la municipalité au centre-ville (tableau \@ref(tab:regfictives)).


```{r regfictives, echo=FALSE, message=FALSE, warning=FALSE, out.width='85%'}
df <- read.csv("data/bivariee/Reg.csv", stringsAsFactors = F)
names(df)[1] <- "Municipalité"
names(df)[2] <- "Vélo"
...
D'emblée, à la lecture du nuage de points (figure \@ref(fig:figreg)), on décèle une forte relative linéaire négative entre les deux variables : plus la distance au centre-ville augmente, plus le pourcentage de cyclistes est faible, ce qui est confirmé par le coefficient corrélation (*r* = −0,86). La droite de régression (en rouge à la figure \@ref(fig:figreg)) qui résume le mieux la relation entre `Vélo` (variable dépendante) et `KmCV` (variable indépendante) s'écrit : **Velo = 30,603 - 1,448 x KmCV**.

La valeur du coefficient de régression ($\beta_{1}$) est de −1,448. Le signe de ce coefficient traduit ainsi une relation négative entre les deux variables. Ainsi, à chaque ajout d'une unité de la distance au centre-ville, le pourcentage de cyclistes diminue de −1,448. Par conséquent, si la distance au centre-ville n'était pas exprimée en kilomètre, mais plutôt en mètres, $\beta_1$ sera égal à −1448.
...
```{r figreg, echo=FALSE, message=FALSE, warning=FALSE, fig.align='center', fig.cap="Relation linéaire entre l'utilisation du vélo et la distance au centre-ville", out.width='75%'}
library(ggplot2)
modele <- lm(Vélo~KmCV, df)
...
pearson <- round(cor(df$Vélo, df$KmCV),3)

ggplot(df, aes(x=KmCV, y=Vélo)) +
...
Reste à savoir comment sont estimées les différents paramètres de l'équation, soit $\beta_0$ et $\beta_1$. À la figure \@ref(fig:figreg2), les points noirs représentent les valeurs observées ($y_i$) et les points bleus les valeurs prédites ($\widehat{y_i}$) par l'équation du modèle. Les traits noirs verticaux représentent pour chaque observation $i$, l'écart entre la valeur observée et la valeur prédite, dénommée résidu ($\epsilon_i$). Si un point est au-dessus de la droite de régression, la valeur observée sera alors supérieure à la valeur prédite ($y_i > \widehat{y_i}$) et inversement si le point est au-dessous de la droite ($y_i < \widehat{y_i}$). Cela signifie que plus cet écart,c'est-à-dire le résidu $\epsilon_i$, est important, plus l'observation s'éloigne de la prédiction du modèle. Au tableau \@ref(tab:regfictives2), vous constatez que la somme des résidus est égale à zéro. La méthode des moindres carrés ordinaires vise alors à minimiser les écarts au carrés entre les valeurs observées ($y_i$) et prédites ($\beta_0+\beta_1 x_i$, soit $\widehat{y_i}$).
...
\begin{equation} 
\beta_0 = \widehat{Y}-\beta_0 \widehat{X}
(\#eq:b0)
\end{equation}


```{r figreg2, echo=FALSE, message=FALSE, warning=FALSE, fig.align='center', fig.cap="Droite de régression, valeurs observées, prédites et résidus", out.width='85%'}
df <- read.csv("data/bivariee/Reg.csv", stringsAsFactors = F)
...
#### Régression polynomiale {#sect3143}

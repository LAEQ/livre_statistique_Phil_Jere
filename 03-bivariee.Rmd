...
L'association entre deux variables peut être simplement le fruit du hasard. Si l'on explore de très grandes quantités de données (avec un nombre impressionnant d'observations et de variables), soit une démarche relevant du *data mining*, le hasard fera que l'on risque d'obtenir des corrélations surprenantes entre certaines variables. Par exemple, on pourrait conclure que les personnes dont le rouge est la couleur préférée aiment le chocolat noir et celles le bleu le chocolat au lait, sans qu'on soit en mesure d'identifier un mécanisme de causalité !

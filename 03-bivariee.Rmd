...
Lorsque les variables sont fortement anormalement distribuées, il est conseillé d'utiliser deux statistiques non-paramétriques : principalement, le coefficient de corrélation de Spearman (_rho_) et secondairement, le coefficient de Kendall ($\tau$, prononcez Tau) qui varient aussi tous deux de −1 à 1. 
...
**Comment expliquer et prédire une variable continue en fonction d'une autre variable?** Répondre à cette question relève de la statistique inférentielle. Il s'agit en effet d'établir une équation simple du type $Y = a + bX$, pour expliquer, puis prédire les valeurs d'une variable dépendante (*Y*) à partir d'une variable indépendante (*X*). Il y a donc un échantillon dit d'apprentissage sur lequel l'équation du modèle de régression est déterminé; et éventuellement, un échantillon pour lequel les valeurs de la variable *Y* sont inconnues, mais qui peuvent être aisément prédites à partir de l'équation du modèle. La régression linéaire simple se distingue ainsi de la **covariance** (section \@ref(sect312)) et la **corrélation** (section \@ref(sect313)) relevant de la statistique bivariée descriptive et exploratoire. 

Par exemple, la régression linéaire simple pourrait être utilisée pour expliquer les notes d'un groupe d'étudiants à un examen (variable dépendante) en fonction du nombre d'heures qu'ils ont consacrés à la révision des notes de cours (variable indépendante). Une fois l'équation de régression déterminée et si le modèle est efficace, nous pourrons prédire précisément les notes des étudiants inscrits au cours la session suivante, et ce, avant même qu'ils aient passé l'examen. 

Tentons de formuler un exemple d'application de la régression linéaire simple en études urbaines. Dans le cadre d'une étude sur les îlots de chaleur urbains, la température de surface (variable dépendante) pourrait être expliquée par la proportion de la superficie de l'îlot couverte par de la végétation (variable indépendante). On supposerait alors que plus cette proportion est importante, plus la température est faible et inversement, soit une relation linéaire négative. Si le modèle est efficace, nous pourrions prédire la température moyenne des îlots d'une autre municipalité pour laquelle nous ne disposons pas d'une carte de température, et repérer ainsi les îlots de chaleur potentiels. Bien entendu, il est peu probable que nous arrivions pas à prédire efficacement la température moyenne des îlots avec uniquement la couverture végétale comme variable explicative. En effet, bien d'autres caractéristiques de la forme urbaine peuvent influencer ce phénomène comme la densité du bâti, la couleur des toits, les occupations du sol présentes, l'effet des canyons urbains, etc. Il faudrait alors inclure non pas une, mais plusieurs variables explicatives (indépendantes).
...
L'association entre deux variables peut être simplement le fruit du hasard. Si l'on explore de très grandes quantités de données (avec un nombre impressionnant d'observations et de variables), soit une démarche relevant du *data mining*, le hasard fera que l'on risque d'obtenir des corrélations surprenantes entre certaines variables. Par exemple, on pourrait conclure que les personnes qui aiment le rouge préfèrent le chocolat noir et celles le bleu le chocolat au lait, sans qu'on soit capable de propose un mécanisme de causalité !
...
Reste à savoir comment sont estimés les différents paramètres de l'équation, soit $\beta_0$ et $\beta_1$. À la figure \@ref(fig:figreg2), les points noirs représentent les valeurs observées ($y_i$) et les points bleus les valeurs prédites ($\widehat{y_i}$) par l'équation du modèle. Les traits noirs verticaux représentent pour chaque observation $i$, l'écart entre le valeur observée et la valeur prédite, dénommée résidu ($\epsilon_i$, prononcez epsilon de _i_ ou plus simplement le résidu pour _i_). Si un point est au-dessus de la droite de régression, la valeur observée sera alors supérieure à la valeur prédite ($y_i > \widehat{y_i}$) et inversement, si le point est au-dessous de la droite ($y_i < \widehat{y_i}$). Cela signifie que plus cet écart,c'est-à-dire le résidu $\epsilon_i$, est important, plus l'observation s'éloigne de la prédiction du modèle. Au tableau \@ref(tab:regfictives2), vous constaterez que la somme des résidus est égale à zéro. La méthode des moindres carrés ordinaires vise alors à minimiser les écarts au carré entre les valeurs observées ($y_i$) et prédites ($\beta_0+\beta_1 x_i$, soit $\widehat{y_i}$) :
...
et qu'elle comprend ainsi une partie de *Y* qui est expliquée par le modèle et une autre partie non expliquée ($\epsilon$ appelé généralement le terme d'erreur). Ce terme d'erreur pourrait représenter d'autres variables explicatives qui n'ont pas été prises en compte pour prédire la variable indépendante. 
...
Autrement dit, la variance totale est donc égale à la variance expliquée plus la variance non expliquée. Au tableau \@ref(tab:computeR), vous pouvez repérer les valeurs de *SCT*, *SCE* et *SCR* et constater que 279,30 = 227,04 + 52,26 ou 27,93 = 22,70 + 5,23.
...
Elle représente ainsi une **mesure absolue des erreurs** qui est exprimée dans l'unité de mesure de la variable dépendante. Dans le cas présent $\sqrt{5,23}=2,29$. Cela signifie qu'en moyenne, l'écart absolu entre les valeurs observées et prédits est de 2,29 points de pourcentage. De nouveau, une plus faible valeur de **RMSE** indique un meilleur ajustement du modèle et surtout avec quelle précision le modèle est capable de prédire la variable dépendante. Il est donc particulièrement important si l'objectif du modèle est de prédire des valeurs sur un échantillon d'observations pour lequel la variable dépendante est inconnue.
...
L'hypothèse nulle (h<sub>0</sub> avec $\beta_1=0$, soit $R^2=0$) est rejetée si la valeur calculée de *F* est supérieure à la valeur critique de la table *F* avec (1, n-2) degrés de liberté et un seuil $\alpha$ (*p*=0,05 habituellement) (voir le tableau des valeurs critiques de F, section \@ref(annexe2)). Notez qu'on utilise rarement la table *F* puisqu'avec la fonction `pf(f calculé, 1, n-2, lower.tail = FALSE)` l'on obtient directement la valeur de *p* associée à la valeur de *F*. Concrètement, si le test F est significatif (avec *p*<0,05), plus la valeur de *F* sera élevée, plus le modèle sera efficace (et plus le $R^2$ sera aussi élevé).

#### Régression polynomiale {#sect3144}

#### Mise en oeuvre dans ![](images/Rlogo.png) {#sect3145}
...
Pour mettre en oeuvre des régressions polynomiales, on utilisera la fonction `poly(x = variable indépendante, degree = nombre de degrés pour la polynomiale)`.

```{r regpoly, echo=TRUE, message=FALSE, warning=FALSE, out.width='85%', fig.cap="Exemple de régression polynomiale (ordre 1 à 4)",}
library(ggplot2)
library(ggpubr)

df <- read.csv("data/bivariee/sr_rmr_mtl_2016.csv")
df <- subset(df, Hab1000Km2< 20) 

# Avec poly(x =, degree =)
# le paramètre degree permet ainsi fixer l'ordre de la polynomiale
summary(lm(Locataire ~ Hab1000Km2, df))
summary(lm(Locataire ~ poly(Hab1000Km2,2), df))
summary(lm(Locataire ~ poly(Hab1000Km2,3), df))
summary(lm(Locataire ~ poly(Hab1000Km2,4), df))

plot1 <- ggplot(df, aes(x = Hab1000Km2, y = Locataire)) +
         geom_point() + 
         stat_smooth(method = "lm", formula = y ~ x, size = 1)

plot2 <- ggplot(df, aes(x = Hab1000Km2, y = Locataire)) + 
         geom_point() + 
         stat_smooth(method = "lm", formula = y ~ poly(x, 2), size = 1)

plot3 <- ggplot(df, aes(x = Hab1000Km2, y = Locataire)) + 
          geom_point() + 
          stat_smooth(method = "lm", formula = y ~ poly(x, 3), size = 1)

plot4 <- ggplot(df, aes(x = Hab1000Km2, y = Locataire)) + 
  geom_point() + 
...
#### Comment rapporter une régression linéaire simple {#sect3146}

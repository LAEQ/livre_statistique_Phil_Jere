...
L’analyse de variance (ANOVA) est largement utilisée en psychologie, médecine et pharmacologie. Prenons un exemple classique en pharmacologie pour tester l'efficacité du médicament. Quatre groupes de population sont constitués :
...
La variable continue permettra d'évaluer l'évolution de l'état de santé des individus (par exemple, la variation du taux de globules rouges dans le sang avant et après le traitement). On s'attendrait alors que les moyennes du deuxième, troisième et quatrième groupes soient plus élevées que celle du groupe de contrôle. Statistiquement parlant, si les moyennes sont significativement différentes ou autrement dit, si l’état de santé s’est nettement plus amélioré chez les membres des second et troisième et quatrième groupes comparativement à celui des membres du groupe contrôle, le médicament est efficace. Les différences de moyennes entre les second, troisième et quatrième groupe permettront aussi de repérer quel dosage est le plus efficace.
...
* la **variance intragroupe**  (*Var~intra~*) ou variance non expliquée (*VNE*) ou , soit la somme des variances des groupes de la variable indépendante (équation \@ref(eq:anova3).
...
Autrement dit, plus la variance intergroupe (**dissimilarité** des groupes) est maximisée et corollairement plus la variance intragroupe (**homogénéité** de chacun des groupes) est minimisée, plus l'ANOVA sera performante.
:::
...
Pour chaque observation, la troisième colonne du tableau représente la dévation à la grande moyenne mise au carré, tandis que les colonnes suivantes représentent la dévation au carré de chaque observation à la moyenne de son groupe d'appartenance. En guise, pour la première observation, on a : $(16,900 - 17,339)^2 = 0,193$ et $(16,900 - 17,580)^2~ = 0,46$. La variance totale (*VT*) est donc égale à la somme de la troisième colonne (424,663), tandis que la variance intragroupe (non expliquée, VNE) est égale à $11,228+21,537+13,993=46,758$. Quant à la variance intergroupe (expliquée, VE), elle est égale à $5\times(17,580-17,339)^2+7\times(12,257-17,339)^2+6\times(23,067-17,339)^2 = 377,904$.
...
avec $n$ et $k$ étant respectivement les nombres d'observations et de modalités de la variable qualitative. L'hypothèse nulle (les moyennes sont égales) sera rejetée si la valeur du *F* calculé est supérieure à la valeur critique de la table *F* avec *(k-1, n-k)* et un seuil 
$\alpha$ (*p*=0,05 habituellement) (voir le tableau des valeurs critiques de *F*, section \@ref(annexe2)). Notez qu'on utilise rarement la table *F* puisqu'avec la fonction `aov` calcule directement la valeur *F* et celle de *p* qui lui est associée. Concrètement, si le test _F_ est significatif (avec *p*<0,05), plus la valeur de *F* sera élevée, plus les moyennes seront statistiquement différentes.
...
* pour le cas A, le F calculé est égal à $F = (377,904 /2) / (46,758 / 15) = 60,62$. Il est  supérieur à la valeur F critique; les moyennes sont donc statistiquement différentes avec *p*<0,05.
* pour le cas B, le F calculé est égal à $F = (6,087/2)/ (115,669 / 15) = 0,39$. Il est inférieur à la valeur F critique; les moyennes ne sont donc pas statistiquement différentes.

#### Conditions d'application de l'ANOVA et solutions alternatives {#sect3414}
Trois conditions d'application doivent être vérifiées avant de calculer une analyser de variance sur un jeu de données :

* **Normalité des groupes.** Le test de Fisher nécessite que les échantillons (groupes) doivent être idéalement normalement distribués. Pour le vérifier, on a recours au test de normalité de Shapiro–Wilk (REF CHAPITRE 1 à METTRE PLUS TARD). Une autre option de vérifier la normalité des résidus, toujours avec le test de Shapiro–Wilk. Notez toutefois que ce test est très restrictif, notamment pour des grands échantillons. Par conséquent, la condition de la normalité est particulièrement importante quand les échantillons sont de taille réduite; corollairement, elle ne doit être appliquée avec sévérité lorsque l'on analyse de grands échantillons. En effet, de nombreux auteurs s'entendent sur le fait que l'ANOVA peut être utilisée même si les distributions des groupes sont légèrement anormalement distribuées. 
...
* Lorsque les échantillons sont fortement anormalement distribués, certains auteurs vont simplement transformer leur variable continue en *log* et reporter le test de fisher calculé sur cette transformation. Attention toutefois ! La transformation logarithme n'est pas une panacée puisqu'elle ne corrige pas systématiquement l'anormalité d'une distribution. Par conséquent, avant de recalculer votre test *F*, il convient de réaliser un test de normalité de Shapiro–Wilk et un test d'homoscédasticité (Levene, Bartlett ou/et Breusch-Pagan) sur la variable continue transformée en logarithme.

* Détecter à l'aide de graphiques, les observations qui contribuent le plus à l'anormalité et l'hétéroscédasticité, dites valeurs aberrantes (*outliers* en anglais). Supprimez les et refaites votre ANOVA, en vous assurant que les conditions sont désormais respectées. J'avoue ne pas être féru de cette solution. Si vos échantillons sont bien constitués et que la mesure collectée n'est pas erronée, pourquoi donc la supprimer? Si vous optez pour cette solution, prenez soin de comparer les résultats avant et après la suppression des valeurs aberrantes. Si les conditions sont respectées après suppression et que les résultats de l'ANOVA (Eta^2^ et test *F* de Fisher) sont très semblables, conservez donc les résultats l'ANOVA intitiale.
...
setwd("D:/Articles et colloque/Livre en cours/AnalysesQuanti/Livre/livre_statistique_Phil_Jere/data/bivariee")
...
GraphViol <- ggplot(df_TrajetsDuree, aes(x=Mode, y=DureeMinute)) +
...
ggarrange(GraphDens, GraphViol)

``` 

Pour vérifier si les échantillons sont normalement distribués avec la fonction `shapiro_test` du package **rstatix**. À titre de rappel, l'hypothèse nulle (h<sub>0</sub>) de ce test est que la distribution est normale. Par conséquent, quand la valeur de *P* associée à la statistique de Shapiro est supérieure à 0,05 alors la distribution est normale. À la lecture des sorties ci-dessous, seul le groupe des utilisateurs en transport en commun présente une distribution normale (p=0,0504). Ce test étant très restrictif, il est fortement conseillé de visualiser le graphique QQ plot (figure \@ref(fig:figQqplot)). Ces graphiques rélèvent que la quasi-totalité des observations sont dans l'intervalle de confiance (en gris), excepté deux observations pour le vélo et deux autres pour l'automobile. Par conséquent, on peut considérer que ces trois distributions s'approchent d'une situation de normalité.
...
Pour vérifier l’hypothèse d’homogénéité des variances, vous pouvez utiliser les tests de Levene, de Bartlett ou de Breusch-Pagan. Les valeurs de *P*, toutes supérieures à 0,05, signalent que la condition d'homogénéité des variances est respectée. La valeur de *P* associée à la statistique *F* (0,444) nous permet de conclure qu'il n'y a pas de différences significatives entre les moyennes des temps de déplacements des trois modes de transport.
...
# Graphique en violons
GraphViol <- ggplot(df_Bruit, aes(x=Mode, y=laeq)) +
  geom_violin(fill="white") +
...
Le test de Shapiro et les graphiques QQ plot (figure \@ref(fig:figQqplot2)) rélèvent que les distributions des trois groupes sont anormales.
...
* **L'analyse de covariance** (**ANCOVA**, **AN**alysis of **COVA**riance en anglais) comprend une variable dépendante continue, une variable indépendante qualitative (facteurs) et plusieurs variables indépendantes continues dits covariables. Concrètement, elle représente une combinaison entre une analyse de variance et une régression. L'objectif est alors de vérifier si les moyennes d'une variable dépendante sont différentes pour plusieurs groupes d'une population donnée, après avoir contrôlé une ou plusieurs variables continues. Par exemple, pour une métropole donnée, on pourra vouloir comparer les moyennes de loyers entre la ville-centre et la première et de la seconde couronnes (facteur), une fois contrôlé la taille de ces derniers (variable covariée continue). En effet, une partie de la variance des loyers s'explique certainement par la taille des logements.

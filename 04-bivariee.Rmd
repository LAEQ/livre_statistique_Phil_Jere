...
Plus spécifiquement, nous présentrons puis mettrons en œuvre dans le logiciel ![](images/logos/Rlogo.png) les méthodes suivantes : covariance, corrélation et régression linéaire simple (entre deux variables quantitatives, section \@ref(sect41)), tableau de contingence et test du khi^2^ (entre deux variables qualitatives, section \@ref(sect42)), t de student (test *t*) et test de Wilcoxon (entre une variable quantitative et une variable qualitative comprenant deux modalités, section \@ref(sect43)), et analyse de variance et test de Kruskal-Wallis (entre une variable quantitative et une variable qualitative comprenant plus de deux modalités, section \@ref(sect44)).
...
## Relation linéaire entre deux variables quantitatives {#sect41}
...
Pour évaluer la linéarité entre deux variables continues, deux statistiques descriptives sont utilisées : la **covariance**\index{covariance} (section \@ref(sect412)) et la **corrélation**\index{corrélation} (section \@ref(sect413)).
:::


### Bref retour sur le postulat de la relation linéaire {#sect411}
...
Dans les sous-sections suivantes, nous décrirons deux statistiques descriptives et exploratoires – la covariance (section \@ref(sect412)) et la corrélation (section \@ref(sect413)) – utilisées pour évaluer la **relation linéaire** entre deux variables continues. Ces deux mesures permettent de mesurer le degré d'association entre deux variables, sans que l'une soit la variable dépendante (variable à expliquer) et l'autre, la variable dépendante (variable explicative). Puis, nous décrirons la régression linéaire simple (section \@ref(sect414))  qui permet justement de prédire une variable dépendante (_Y_) à partir d'une variable indépendante (_X_).

### Covariance {#sect412}

#### Formulation {#sect4121} 
...
#### Corrélations pour des variables anormalement distribuées (coefficients de Spearman, Tau de kendall) {#sect4133} 
...
#### Corrélations robustes (*Biweight midcorrelation*, *Percentage bend correlation* et la corrélation *pi* de Shepherd) {#sect4134}
...
#### Significativité des coefficients de corrélation {#sect4135} 
...
#### Corrélation partielle {#sect4136} 
...
#### Mise en œuvre dans ![](images/Rlogo.png) {#sect4137}
...
#### Comment rapporter des valeurs de corrélations? {#sect4138}
...
### Régression linéaire simple  {#sect414}
...
La régression linéaire simple se distingue ainsi de la **covariance** (section \@ref(sect412)) et de la **corrélation** (section \@ref(sect413)), relevant de la statistique bivariée descriptive et exploratoire. 
...
#### Formulation de la droite de régression des moindres carrés ordinaires {#sect4142}
...
#### Mesurer la qualité d'ajustement du modèle {#sect4143}
...
#### Mise en œuvre dans ![](images/Rlogo.png) {#sect4144}
...
#### Comment rapporter une régression linéaire simple {#sect4145}
...
## Relation entre deux variables qualitatives {#sect42}
...
Pour vérifier l'existence d'associations significatives entre les modalités de deux variables qualitatives, il est possible de construire un **tableau de contingence**\index{tableau de contingence} (section \@ref(sect421)), puis de réaliser le **test du khi^2^**\index{khi2} (section \@ref(sect422)).
:::


### Construction de tableau de contingence {#sect421}
...
### Mise en œuvre dans ![](images/Rlogo.png) {#sect424}
...
### Interprétation d'un tableau de contingence {#sect425}
...
## Relation entre une variable quantitative et une variable qualitative à deux modalités {#sect43}
...
### Test *t* et ses différentes variantes {#sect431}
...
#### Principe de base et formulation pour des échantillons dépendants (appariés) {#sect4311}
...
#### Mesurer la taille de l'effet {#sect4312}
...
#### Mise en œuvre dans ![](images/Rlogo.png) {#sect4313}
...
#### Comparer des moyennes pondérées {#sect4314}
...
#### Comment rapporter un test de student ou de Welch? {#sect4315}
...
### Test non paramétrique de Wilcoxon {#sect432}
...
## Relation entre une variable quantitative et une variable qualitative à plus de deux modalités {#sect44}
...
#### Le calcul des trois variances pour l'ANOVA {#sect4411}
...
#### Conditions d'application de l'ANOVA et solutions alternatives {#sect4414}
...
### Test non paramétrique de Kruskal-Wallis {#sect442}
...
### Mise en œuvre dans ![](images/Rlogo.png) {#sect443}
...
# Compairison des groupes avec la fonction pairwise.wilcox.test
pairwise.wilcox.test(df_Bruit$laeq, df_Bruit$Mode, p.adjust.method = "BH")
```

### Comment rapporter les résultats d'une ANOVA et du test de Kruskal-Wallis {#sect444}

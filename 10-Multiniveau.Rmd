...
## Modèles multiniveaux&nbsp;: deux intérêts majeurs {#sect101}
...
### Répartition de la variance entre les différents niveaux {#sect1011}
...
### Estimation des coefficients aux différents niveaux {#sect1012}
...
## Différents types de modèles multiniveaux {#sect102}
...
Dans ce second modèle, nous introduisons uniquement les variables explicatives au niveau&nbsp;1. Par conséquent, ce modèle est tout simplement un modèle à effets mixtes (GLMM) avec des constantes aléatoires largement décrit à la section \@ref(sect0921)). Si vous comparez l'équation du modèle vide (équation \@ref(eq:multiniveauIcc)) avec l'équation de ce modèle (équation \@ref(eq:multiniveauModele2)), vous constaterez que le paramètre $\beta_1 x_1$ a été ajouté. Il correspond au coefficient pour la variable indépendante $X_1$ mesurée au niveau&nbsp;1 (effet fixe). Nous pourrions alors ajouter d'autres paramètres pour les autres variables indépendantes du modèle, soit $\beta_1 x_1 + \beta_2 x_2+ \ldots + \beta_k x_k$ ($k$ étant le nombre de variables explicatives mesurées au niveau&nbsp;1, effets fixes).
...
Le troisième type de modèle consiste à introduire à la fois les variables explicatives mesurées au niveau&nbsp;1 et au niveau&nbsp;2 (équation \@ref(eq:multiniveauModele3)). Il est communément appelé le modèle complet. Les variables explicatives du niveau&nbsp;2 sont aussi considérées comme des effets fixes.
...
### Modèle avec les variables indépendantes du niveau&nbsp;1 {#sect1042}
...
### Modèle avec les variables indépendantes aux niveaux 1 et 2 {#sect1043}
...
### Modèle complet avec une interaction {#sect10414}
...
### Comparaison des quatre modèles {#sect1045}
...
## Quiz de révision du chapitre {#sect105}

```{r quizChapitre10, echo=FALSE, warning=FALSE, results="asis"}
Chapitre10_Multiniveau <- quizz("quizzs/Chapitre10_Multiniveau.yml", "Chapitre10_Multiniveau")

...
```{r fig35, fig.align='center', auto_pdf = TRUE, fig.cap="Ajout d'annotations textuelles",  out.width='65%', dev = "png", dpi = 300}
ggplot() +
  geom_point(mapping = aes(x = Sepal.Length, y = Sepal.Width), data = iris) +
  labs(title = "Morphologie des sépales des iris", subtitle = "n = 150",
       x = "Longueur des sépales", 
...
```{r fig36, fig.align='center', auto_pdf = TRUE, fig.cap="Ajout d'annotations géométriques",  out.width='65%', dev = "png", dpi = 300}
...
```{r fig37, fig.align='center', auto_pdf = TRUE, fig.cap="Gestion de l'ordre des annotations",  out.width='65%', dev = "png", dpi = 300}
ggplot() +
  annotate("rect", 
           ymin = sepal.width_extent[[1]],
           ymax = sepal.width_extent[[2]],
...
* Le thème classique (`theme_classic`) (figure \@ref(fig:fig38))
...
* Le thème gris (`theme_gray`) (figure \@ref(fig:fig39))
...
* Le thème noir et blanc (`theme_bw`) (figure \@ref(fig:fig310))
...
* Le thème minimal (`theme_minimal`) (figure \@ref(fig:fig311))
...
* Le thème *tufte* (`theme_tufte`, à l'ancienne...) (figure \@ref(fig:fig312))
...
* Le thème *economist* (`theme_economist`, inspiré de la revue du même nom) (figure \@ref(fig:fig313))
...
* Le thème *solarized* (`theme_solarized`, plus original) (figure \@ref(fig:fig314))
...
### Composition d'une figure avec plusieurs graphiques {#sect0314}
...
Nous pourrions souhaiter réaliser une figure composite avec le jeu de données *iris* et séparer notre nuage de points en trois graphiques distincts selon l'espèce des iris (figure \@ref(fig:fig315)). Pour cela, il faut au préalable convertir la variable *espèce* en facteur.
...
```{r fig316, fig.align='center', auto_pdf = TRUE, fig.cap="Graphique à facettes en une ligne",  out.width='65%', dev = "png", dpi = 300}
ggplot() +
  geom_point(mapping = aes(x = Sepal.Length, y = Sepal.Width), data = iris) +
  labs(title = "Morphologie des sépales des iris", subtitle = "n = 150",
       x = "Longueur des sépales", 
...
#### Arrangement des graphiques {#sect03142}
...
* En utilisant le nom de la couleur dans une chaîne de caractère&nbsp;: `"chartreuse4"`. R dispose de 657&nbsp;noms de couleurs prédéfinis. Pour tous les afficher, utilisez la fonction `colors()`, qui permet de les visualiser (figure \@ref(fig:figcolors)).
...
#### Histogramme simple {#sect03211}

Générons quatre variables ayant respectivement une distribution gaussienne, Student, Gamma et bêta, puis réalisons un histogramme pour chacune de ces variables et combinons-les avec la fonction `ggarrange` (figure \@ref(fig:fig318)).

```{r fig318, fig.align='center', fig.cap="Histogrammes", message=FALSE, warning=FALSE, auto_pdf=TRUE, dev="png", dpi=300, out.width='65%'}
...
```{r fig320, fig.align='center', fig.cap="Histogrammes à facettes", message=FALSE, warning=FALSE, auto_pdf=TRUE, out.width='65%', dev = "png", dpi = 300}
library(reshape2)
...
#### Histogramme de densité {#sect03212}
...
```{r fig324, fig.align='center', auto_pdf = TRUE, fig.cap="Graphiques de densité à facette",  out.width='65%', dev = "png", dpi = 300}
ggplot(data = melted_distribs)+
...
```{r fig331, fig.align='center', fig.cap="Densité lissée en deux dimensions", warning=FALSE, auto_pdf=TRUE, out.width='65%', dev = "png", dpi = 300}
ggplot(data = data_co2)+
...
```{r fig339, fig.align='center', fig.cap="Boîtes à moustaches améliorées", message=FALSE, warning=FALSE, auto_pdf=TRUE, out.width='65%', dev = "png", dpi = 300}
ggplot(data = data_co2_comp)+
  geom_boxplot(aes(y = region7, x = CO2t_hab))+
  xlim(c(0,50))+
...
```{r fig345, fig.align='center', auto_pdf = TRUE, fig.cap="Graphique en barre empilée",  out.width='65%', dev = "png", dpi = 300}
...
```{r fig346, fig.align='center', fig.cap="Graphique en tarte", message=FALSE, warning=FALSE, auto_pdf=TRUE, out.width='65%', dev = "png", dpi = 300}
library(dplyr)
...
```{r fig348, fig.align='center', fig.cap="Graphique en anneau", message=FALSE, warning=FALSE, auto_pdf=TRUE, dev="png", dpi=300, out.width='65%'}
library(fmsb)
...
## Exportation des graphiques {#sect035}

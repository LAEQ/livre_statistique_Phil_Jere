...
L'histogramme permet de décrire graphiquement la forme de la distribution d'une variable. Pour le réaliser, on utilise la fonction `geom_histogram`. Le paramètre le plus important est le nombre de barre (`bins`) qui composent l'histogramme. Plus ce nombre est grand, plus l'histogramme est précis et à l'inverse, plus il est petit, plus l'histogramme est simplifié. Par contre, il convient de ne pas recourir à un nombre de barres trop élevée comparativement au nombre d'observations disponibles dans le jeu de données.

#### histogrammes simples

Générons quatre variables ayant respectivement une distribution gaussienne, Student, Gamma et Beta, puis réalisons un histogramme pour chacune de ces variables avec la fonction `ggarrange`.
...
Il s'agit d'une petite île néerlandaise des Caraïbes nommée Aruba disposant d'une faible population mais avec des activités très polluantes (raffinerie et extraction d'or). Nous faisons ici le choix de retirer ces observations puisqu'elles sont assez peu représentatives de la tendance mondiale. <span class="revision_rem">Cette démarche si simple relève ainsi de l'analyse exploratoire des données !</span>
...
Le message du graphique est le même. Notez que nous avons utilisé ici la fonction `guides` pour retirer de la légende les couleurs associées au remplissage des marges d'erreurs. Ces couleurs sont les mêmes que celles des lignes et il n'est pas utile de dédoubler la légende. Notez que de nombreuses méthodes statistiques produisent des résultats accompagnés d'une mesure de l'incertitude associée à ces résultats. Représenter cette incertitude est crucial pour que le lecteur puisse délimiter la portée des conclusions.
...
data_co2_comp2 <- subset(data_co2, (! data_co2$region7 %in% c("Sub-Saharan Africa", "South Asia")) 
...
Si à la place de la géométrie `geom_bar`, vous utilisez `geom_rect`, vous pouvez convertir votre graphique en pointe de tarte en graphique en anneau (ou en beigne, de nouveau pour les dents sucrés) : 
...
```
Le graphique permet de remaquer que la plupart des flux s'effectuent au sein d'un même secteur. On peut cependant constater que les secteurs des arrondissements des Rivières et la cité Limoilou attirent une plus grande quantité et diversité de flux.

### Word cloud

Un nuage de mots est un graphique utilisé en analyse textuelle pour représenter les mots les plus importants d'un texte. Mesurer l'importance des mots dans un document est une discipline à part entière, nous proposons un simple exemple ici avec la méthode *rake* (*Rapid Automatic Keyword Extraction*) proposée par [@rose2010automatic] et implémentée dans le *package* **tm**. Nous aurons également besoin des *packages* **udpipe** (fournissant des dictionnaires linguistiques), **readtext** (pour lire un fichier *txt*), **RColorBrewer** (pour sélectionner une palette de couleur) et **wordcloud** (pour générer le graphique). <span class="revision_rem">Nous avons choisi comme texte un résumé d'un article en anglais portant sur les méthodes d'accessibilités aux services de santé</span>. <span class="revision_prop">AJOUTER 1 PHRASE OU 2 SUR LES 2 PMAD</span> Notez qu'à chaque génération du nuage de mots, vous obteindrez une disposition différente. N'hésitez pas à en essayer plusieurs jusqu'à trouver celle qui vous semble optimale.
...
# téléchargement du dictionnaire anglais
...
```{r fig355, fig.align='center', echo=FALSE, include=identical(knitr:::pandoc_to(), 'html'), auto_pdf = TRUE, fig.cap="Exporter un graphique dans RStudio",  out.width='40%'}

...
Dans ce chapitre, nous découvrirons les incroyables capacités graphiques de ![](images/logos/Rlogo.png). Pour ce faire, nous couvrirons en profondeur les capacités du *package* **ggplot2** du **tidyverse**. Il s'agit de loin du meilleur *package* pour réaliser des graphiques selon nous.
...
```{r fig3A, echo=FALSE, fig.align='center', fig.cap="Drowning in Plastic par [Reuters Graphics](https://graphics.reuters.com/ENVIRONMENT-PLASTIC/0100B275155/index.html){target='_blank'}", auto_pdf=TRUE, out.width='90%'}
knitr::include_graphics('images/magie_graphiques/Drowning-in-Plastic.jpg', dpi = NA)
```

Ce second graphique représente quatre informations pour 234 villes à travers le monde : 
...
* le continent sur lequel est localisé chaque ville (couleur des cercile).

```{r fig3B, echo=FALSE, fig.align='center', fig.cap="Changement climatique et vulnérabilité par [Verisk Maplecroft](https://www.maplecroft.com/insights/analysis/84-of-worlds-fastest-growing-cities-face-extreme-climate-change-risks/){target='_blank'}", auto_pdf=TRUE, out.width='90%'}
...
A MODIFIER UNE FOIS QU'ON A FINI 
...
* Pour les corrélations ((section \@ref(sect411))) : 
...
* Pour le tableau de contignence ((section \@ref(sect412))) :
  - **gmodels**, pour construire des tableaux de contingence et * Calculer les test *t* et ses différentes variantes ((section \@ref(sect424)))
  - **vcd**, pour construire un graphique pour un tableau de contigence ((section \@ref(sect413)))
* Pour les test *t* : 
  - **sjstats** pour réaliser des test *t* pondérés
  - **effectsize**, pour calculer les tailles d'effet de tests de *t*
* Pour la section sur les ANOVA ((section \@ref(sect414))) : 
...
```{r fig30, fig.align='center', echo=FALSE, include=identical(knitr:::pandoc_to(), 'html'), auto_pdf = TRUE, fig.cap="Trois composantes d'un graphique, adapté de @wickham2010layered",  out.width='70%'}
...
ggplot() +
  geom_point(mapping = aes(x = Sepal.Length, y = Sepal.Width), data = iris) +
  labs(title = "Morphologie de sépals des iris", subtitle = "n = 150",
...
Nous avons montré dans le premier exemple comment rajouter le titre, sous-titre et le nom des axes sur un graphique. Il est également possible de rajouter un texte sous le graphique (généralement la source des données avec l'e paramètre l'argument `caption`) et des annotations textuelles (`annotate`). Pour ces dernières, il convient de spécifier leur localisation (cordonnées `x` et `y`) et le texte à intégrer (`label`); elles sont ensuite ajoutées au graphique avec l'opérateur `+`. Ajoutons deux annotations pour identifier deux fleurs spécifiques.
...
### Composer une figure avec plusieurs graphiques
...
```{r fig317, fig.align='center', include=identical(knitr:::pandoc_to(), 'html'), auto_pdf = TRUE, fig.cap="Figure avec plusieurs graphiques avec ggarrange",  out.width='65%'}
library(ggpubr)
...
Dans un graphique, la couleur peut être utilisée à la fois pour représenter une variable quantitative (dégradé de couleur ou discrétisation), ou une variable qualitative (couleur par catégorie). Dans **ggplot2**, il est possible d'attribuer une couleur au contour des géométries avec l'argument `color` et au remplissage d'une géométrie avec l'argument `fill`. Il est possible de spécifier une couleur de trois façons dans ![](images/logos/Rlogo.png) : 

* En utilisant le nom de la couleur dans une chaîne de caractère : `"chartreuse4"`. ![](images/logos/Rlogo.png) Dispose 657 noms de couleurs prédéfinis. Pour tous les afficher, utilisez la fonction `colors()`, la figure \@ref(fig:figcolors)) permet de les visualiser.

```{r figcolors, fig.align='center', include=identical(knitr:::pandoc_to(), 'html'), auto_pdf = TRUE, fig.cap="Couleurs de base",  out.width='85%'}
knitr::include_graphics('images/magie_graphiques/all_colors.png', dpi = NA)
```

* En indiquant le code hexadécimal de la couleur. Il s'agit d'une suite de 6 lettres et de chiffres précédée par un dièse : `"#99ff33"`
...
Le choix des couleurs est un problème plus complexe que la manière de les spécifier. Il existe d'ailleurs tout un pan de la sémiologie graphique dédié à la question du choix et de l'association des couleurs. Une première ressource intéressante est  [ColorBrewer](https://colorbrewer2.org/#type=sequential&scheme=BuGn&n=3){target="_blank"}. Il s'agit d'une sélection de palettes de couleurs particulièrements efficaces et dont certaines sont mêmes adaptées pour les personnes daltoniennes. Il est possible d'accéder directement aux palettes dans ![](images/logos/Rlogo.png) grâce au package **RColorBrewer** et la fonction `brewer.pal` : 

```{r figcolorBrewer, fig.align='center', include=identical(knitr:::pandoc_to(), 'html'), auto_pdf = TRUE, fig.cap="palette de couleurs de ColorBrewer",  out.width='85%'}
...
Puisque vous êtes désormais initié aux bases de la grammaire des graphiques implémentées par **ggplot2**, vous apprendrez dans les sous-sections suivantes à construire les principaux graphiques que vous utiliserez régulièrement et/ou que vous retrouvez présenté dans un article scientifique.
:::

### Histogramme

L'histogramme permet de décrire graphiquement la forme de la distribution d'une variable. Pour le réaliser, on utilise la fonction `geom_histogram`. Le paramètre le plus important est le nombre de barres (`bins`) qui composent l'histogramme. Plus ce nombre est grand, plus l'histogramme est précis et à l'inverse, plus il est petit, plus l'histogramme est simplifié. En revanche, il faut éviter d'utiliser un nombre de barres trop élevée comparativement au nombre d'observations disponibles dans le jeu de données, sinon votre histogramme sera pleins de trous.
...
```{r fig318, fig.align='center', fig.cap="Histogrammes", message=FALSE, auto_pdf=TRUE, include=identical(knitr:::pandoc_to(), 'html'), out.width='65%'}
...
Notez que cette syntaxe est très lourde. Dans le cas présent, il serait plus judicieux d'utiliser la fonction `facet_wrap`. Pour cela, nous devons au préalable « faire fondre nos données », ce qui signifie changer la forme du *dataframe* actuel qui comprend quatre colonnes (gaussien, gamma, beta et student) et 1000 observations, pour qu'il n'ait plus que deux colonnes (la valeur originale et le nom de l'ancienne colonne) et 4000 observations. La figure \@ref(fig:fig319) décrit graphiquement ce processus qui peut être effectué avec la fonction `melt` du *package* **reshape2**.
...
#### Histogramme avec coloration des valeurs extrêmes

Il peut être nécessaire d'attirer le regard sur certaines partie de l'histogramme, comme par exemple des valeurs extrêmes. Si nous reprenons notre distribution de Student, nous pouvons clairement distinguer un ensemble de valeurs fortes à droite de la distribution. On pourrait dans notre cas considérer que des valeurs au delà de 50 consituent des cas extrêmes que nous souhaitons représenter dans une autre couleur. Pour cela, nous devons créer une variable catégorielle nous permettant de distinguer ces cas particuliers.
...
ggplot(data = distribs) +
  ...
  scale_fill_manual('cas extrêmes', values = c("#a8dadc","#e63946"))+
...
Bien la transparence nous aide un peu à distinguer les secteurs du graphique avec le plus de points, il serait plus efficace d'abandonner la géométrie des points pour la remplacer par une géométrie de densité en deux dimensions. Une première approche consiste à diviser l'espace du graphique en petits carrés et à compter le nombre de points tombant dans chaque carré (un histogramme en deux dimension en somme).
...
#retirer les observations n'étant pas associée à une région
...
Une alternative directe au graphique en barre est le graphique ou diagramme circulaire, appelé aussi graphique en pointe de tarte <span class="revision_prop">(pour les gourmands)</span> ou en camembert <span class="revision_prop">(les amateurs de fromage)</span>. Il est suffisamment connu et utilisé pour qu'aucune présentation ne s'impose. Pour être exact, un graphique en pointe de tarte n'est rien d'autre qu'un graphique en barre dont le système de coordonnées a été modifié. Cela impose cependant de calculer à l'avance la position des étiquettes que l'on souhaite ajouter sur le graphique. Reprenons les données de production mondiale de CO<sub>2</sub> et calculons les productions totales par région géographique en 2015.

```{r fig346, fig.align='center', fig.cap="Graphique en pointe de tarte", message=FALSE, warning=FALSE, auto_pdf=TRUE, include=identical(knitr:::pandoc_to(), 'html'), out.width='65%'}
library(dplyr)
...
Si à la place de la géométrie `geom_bar`, vous utilisez `geom_rect`, vous pouvez convertir votre graphique en pointe de tarte en graphique en anneau (ou en beigne, pour les gourmands) : 

```{r fig347, fig.align='center', include=identical(knitr:::pandoc_to(), 'html'), auto_pdf = TRUE, fig.cap="Graphique en anneau",  out.width='65%'}
...
# la légende est découpée en plusieurs morceau pour gagner de la place 
...
# si vos données sont structurée de la même façon, cette partie du code 
# ne change pas d'un graphique à l'autre
par(mar = c(4, 1, 1, 1))
...
3. Retirer les caractères représentant les sauts de ligne et les sauts de paragraphes (`\n` et `\r`)
4. Concaténer tout le texte en une seule longue chaîne de caractère
5. Utiliser un dictionnaire pour déterminer la nature des mots du texte (nom, adjectifs, verbes, etc.)
...
Notez que toutes ces étapes de nettoyages ne seraient pas nécessaire si nous utilisions un simple fichier texte comme point de départ. Cependant, il est plus courant de rencontrer des fichiers pdf, cet exercice est donc davantage révélateur de la difficulté réelle de la réalisation d'un nuage de mots.
...
# Étape 4 : créer une seule longue chaîne de caractère
...
treemap(data_co2_2015, index=c("region7","region23"),
    ...
    title = "CO2 rejettées par pays en 2015",
    ...
    inflate.labels=F, # agrandir la taille de étiquettes ou non
...
```{r fig353, fig.align='center', fig.cap="Carte thématique avec ggplot2", message=FALSE, warning=FALSE, auto_pdf=TRUE, include=identical(knitr:::pandoc_to(), 'html'), out.width='85%'}
library(sf)
library(classInt)
library(ggsn)
...
```{r fig354, fig.align='center', fig.cap="Carte thématique avec tmap", message=FALSE, warning=FALSE, auto_pdf=TRUE, include=identical(knitr:::pandoc_to(), 'html'), out.width='85%'}
library(tmap)
...
```{r fig355, fig.align='center', fig.cap="Combiner des cartes avec tmap", message=FALSE, warning=FALSE, auto_pdf=TRUE, include=identical(knitr:::pandoc_to(), 'html'), out.width='85%'}
library(tmap)
...
```{r fig356, fig.align='center', echo=FALSE, include=identical(knitr:::pandoc_to(), 'html'), auto_pdf = TRUE, fig.cap="Exporter un graphique dans RStudio",  out.width='40%'}
...
Pour les graphiques n'étant pas réalisés avec **ggplot2**, l'alternative à la fonction `ggsave` est l'ensemble de fonctions `png`, `bmp`, `jpeg`, `tiff` et `pdf`, qui permettent d'exporter n'importe quel graphique dans ces différents formats. <span class="revision_prop">Le processus est cependant un peu complexe et comprend trois étapes C'EST PAS BIEN SORCIER....</span>.<span class="revision_prop">Le processus est cependant comprend trois étapes.</span> 

...
Rappelons que la déviance d’un modèle est une quantité représentant à quel point le modèle est erroné. L’idée de l’indicateur de la déviance expliquée est d’estimer le pourcentage de la déviance maximale observable dans les données que le modèle est parvenu à expliquer. La déviance maximale observable dans les données est obtenue en utilisant la déviance totale du modèle nul (notée $M_n$, soit un modèle dans lequel aucune variable indépendante n’est ajoutée et ne comportant qu’une constante). Cette déviance est maximale puisqu’aucun prédicteur n’est présent dans le modèle. On calcule ensuite le pourcentage de cette déviance totale qui a été contrôlée par le modèle étudié ($M_e$).
...
```{r cookdistGLMbinom, echo=FALSE, fig.align="center", fig.cap="Distances de Cook pour le modèle binomial avec toutes les observations", warning=FALSE, message=FALSE, out.width='60%'}
# Calcul et représentation des distances de Cook
cookd <- data.frame(
  dist = cooks.distance(model1),
  oid = 1:nrow(dfenquete)
...
show_as_png(tplot, out = "images/glm/cookdistGLMbinom.png", dpi = 300)
```
...
```{r cookdistGLMbinom2, echo=FALSE, fig.align="center", fig.cap="Distances de Cook pour le modèle binomial sans les valeurs aberrantes", warning=FALSE, message=FALSE, out.width='60%'}
# Recalculer la distance de Cook
cookd <- data.frame(
  dist = cooks.distance(model2),
  oid = 1:nrow(dfenquete2)
...
show_as_png(tplot, "images/glm/cookdistGLMbinom2.png",300)
```
...
```{r figpaysbinom, message=FALSE, warning=FALSE, fig.align="center", fig.cap= "Rapports de cote pour les différents pays de l'UE", fig.width = "100%"}
...
```{r CookdCotePropor, echo=FALSE, fig.align="center", fig.cap="Distances de Cook pour le modèle logistique des cotes proportionnelles", message=FALSE, warning=FALSE, out.width="65%"}
tplot <- ggarrange(plot1, plot2, ncol = 2, nrow = 1)
show_as_png(tplot, "images/glm/CookdCotePropor.png",300)
```
...
```{r surdisppoiss, message=FALSE, warning=FALSE, fig.aling = "center", fig.cap="Représentation de la sur-dispersion des données dans le modèle de Poisson", out.width='65%'}
...
Les modèles Hurdle sont une autre catégorie de modèles GLM. Ils peuvent être décris avec la formulation suivante&nbsp;: 
...
```{r gausscook, echo=FALSE, fig.align="center", fig.cap="Distances de Cook pour le modèle gaussien", message=FALSE, warning=FALSE, out.width= "65%"}
tplot <- ggplot(data = df) + 
  geom_point(aes(x = oid, y = cook), color = rgb(0.4,0.4,0.4,0.7), size = 1) + 
  labs(x="", y = "distance de Cook")
show_as_png(tplot, "images/glm/gausscook.png",300)
```
...
```{r gausscook2, echo=FALSE, fig.align="center", fig.cap="Distances de Cook pour le modèle gaussien après suppression des observations influentes", message=FALSE, warning=FALSE, out.width="65%"}
tplot <- ggplot(data = df) + 
  geom_point(aes(x = oid, y = cook), color = rgb(0.4,0.4,0.4,0.7), size = 1) + 
  labs(x="", y = "distance de Cook")
show_as_png(tplot, "images/glm/gausscook2.png")
```
...
ggplot() + 
  ...
  labs(x = 'valeurs prédites',
       y = "valeurs réelles")
```
```{r gausssim2, echo=FALSE, fig.align="center", fig.cap="Comparaison de la distribution originale de la variable et des simulations issues du modèle", message=FALSE, warning=FALSE, out.width="65%"}
tplot <- ggplot() + 
  geom_point(data = df1, 
             mapping = aes(x = mus, y = reals),
             color ="red", size = 0.5) + 
  ...
  labs(x = 'valeurs prédites',
       y = "valeurs réelles")
show_as_png(tplot, "images/glm/gausssim2.png")
```
...
ggplot(data = df) + 
  ...
  labs(x="", y = "distance de Cook")
```
```{r stucookdist, echo=FALSE, fig.align="center", fig.cap="Distances de Cook pour un modèle GLM avec une distribution de Student", message=FALSE, warning=FALSE, out.width="50%"}
# Calcul des distances de Cook
cooksd <- cooks.distance(modele)
# Affichage des valeurs
df <- data.frame(
...
tplot <- ggplot(data = df) + 
  geom_point(aes(x = oid, y = cook), color = rgb(0.4,0.4,0.4,0.7), size = 1) + 
  labs(x="", y = "distance de Cook")
show_as_png(tplot, "images/glm/stucookdist.png")
```
...
ggplot(data = df) + 
  ...
  labs(x="", y = "distance de Cook")
```
```{r stucookdist2, echo=FALSE, fig.align="center", fig.cap="Distances de Cook pour un modèle GLM avec une distribution de Student, warning=FALSE, après suppression des valeurs fortement influentes", message=FALSE, out.width="50%"}
# Calcul des distances de Cook
cooksd <- cooks.distance(modele)
# Affichage des valeurs
df <- data.frame(
...
tplot <- ggplot(data = df) + 
  geom_point(aes(x = oid, y = cook), color = rgb(0.4,0.4,0.4,0.7), size = 1) + 
  labs(x="", y = "distance de Cook")
show_as_png(tplot, "images/glm/stucookdist2.png")
```
...
Nous arrivons ainsi à un total de 24&nbsp;969 observations. Pour modéliser ces durées de déplacement, nous utilisons les prédicteurs présentés dans le tableau \@ref(tab:variablegamma)
...
ggplot(data = df)+
  ...
  labs(x = "", y = "distance de Cook")
```
```{r cookdistgamma, echo=FALSE, fig.align="center", fig.cap="Distances de Cook pour le modèle Gamma", message=FALSE, warning=FALSE, out.width='50%'}
# Représentation des distances de Cook
tplot <- ggplot(data = df)+
  geom_point(aes(x = oid, y = cook), size = 0.5, color = rgb(0.4,0.4,0.4,0.4)) + 
  geom_hline(yintercept = 0.003, color = "red") +
  labs(x = "", y = "distance de Cook")
show_as_png(tplot, "images/glm/cookdistgamma.png")
```
...
tplot <- ggplot(data = df)+
  geom_point(aes(x = oid, y = cook), size = 0.5, color = rgb(0.4,0.4,0.4,0.4)) +
  labs(x = "", y = "distance de Cook")
show_as_png(tplot, "images/glm/cookdistgamma2.png")
```
...
tplot <- ggplot() + 
  ...
  labs(x = 'valeurs prédites',
       y = "valeurs réelles")
show_as_png(tplot, "images/glm/dispgamma.png", 300)
```
...
ggplot(data = df)+
  ...
  labs(x = "", y = "Distance de Cook")
```
```{r cookdistbeta, echo=FALSE, fig.align="center", fig.cap="Distances de Cook pour le modèle GLM Beta", message=FALSE, warning=FALSE, out.width="65%"}
# Affichage des distances de Cook
tplot <- ggplot(data = df)+
  geom_point(aes(x = oid, y = cooksd),
             color = rgb(0.4,0.4,0.4,0.4), size = 0.5)+
  labs(x = "", y = "Distance de Cook")
show_as_png(tplot, "images/glm/cookdistbeta.png",300)
```
...
ggplot(data = df2)+
  ...
  labs(x = "", y = "Distance de Cook")
```
```{r cookdistbeta2, echo=FALSE, fig.align="center", fig.cap="Distances de Cook pour le modèle GLM Beta (suppression de deux observations influentes)", message=FALSE, warning=FALSE, out.width="65%"}
# Affichage des distances de Cook
tplot <- ggplot(data = df2)+
  geom_point(aes(x = oid, y = cooksd),
             color = rgb(0.4,0.4,0.4,0.4), size = 0.5)+
  labs(x = "", y = "Distance de Cook")
show_as_png(tplot, "images/glm/cookdistbeta2.png")
```
...
ggplot(data = df3)+
  ...
  labs(x = "", y = "Distance de Cook")
```
```{r cookdistbeta3, echo=FALSE, fig.align="center", fig.cap="Distances de Cook pour le modèle GLM Beta (suppression de trois observations influentes)", message=FALSE, warning=FALSE, out.width="65%"}
# Affichage des distances de Cook
tplot <- ggplot(data = df3)+
  geom_point(aes(x = oid, y = cooksd),
             color = rgb(0.4,0.4,0.4,0.4), size = 0.5)+
  labs(x = "", y = "Distance de Cook")
show_as_png(tplot, "images/glm/cookdistbeta3.png")
```

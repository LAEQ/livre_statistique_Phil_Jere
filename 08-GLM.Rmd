...
Par exemple, admettons que nous ayons eu un coefficient $\beta_1$ de 1,2 pour une variable $X_1$ dans une régression logistique. Il est nécessaire d'utiliser son exponentiel pour l'interpréter de façon intuitive. $\mbox{exp(1,2)} = \mbox{3,32}$, ce qui signifie que lorsque $X_1$ augmente d'une unité, les chances d'observer 1 plutôt que 0 comme valeur de *Y* sont multipliées par 3,32. Admettons maintenant que $\beta_1$ vaille −1,2, nous calculons donc $\mbox{exp(-1,2) = 0,30}$, ce qui signifie qu'à chaque augmentation d'une unité de $X_1$, les chances  d'observer 1 plutôt que 0 comme valeur de *Y* sont multipliées par 0,30. En d'autres termes,les chances d'observer 1 plutôt que 0 sont divisées par  3,33 ($\mbox{1}/\mbox{0,30} = \mbox{3,33}$), soit une diminution de 70&nbsp;% ($\mbox{1}-\mbox{0,3} = \mbox{0,7}$) des chances d'observer 1 plutôt que 0.
...
## Quiz de révision du chapitre {#sect087}

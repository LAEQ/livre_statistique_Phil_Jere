...
La question de l’excès de zéros a été abordée dans la section \@ref(#02437) présentant les distributions. Il s’agit d’une situation où un plus grand nombre de zéros sont présents dans les données que supposé selon la distribution de Poisson. Dans ce cas, il convient d'utiliser la distribution de Poisson avec excès de zéros. Une façon simple de vérifier si nous avons un trop grand nombre de zéros est de comparer les quantités prédites par le modèle de Poisson avec les quantités réelle (variable *Y*).
...
Pour tenir compte de cette particularité des données, nous modifions légèrement le modèle pour utiliser une distribution de quasi-Poisson, intégrant spécifiquement un paramètre de dispersion. Cet ajustement ne modifie pas l'estimation des coefficients du modèle, mais modifie le calcul des erreurs standards et par extension les valeurs de *p* pour les rendre moins sensibles au problème de surdispersion. Une autre approche aurait été de calculer une version robuste des erreurs standards avec le *package* **sandwich** comme nous l'avons fait dans la section sur le modèle binomial (@ref(sect06211)). Après réajustement du modèle, le nouveau paramètre de dispersion estimé est de 1,92.
...
# Calcul de la deviance expliquee

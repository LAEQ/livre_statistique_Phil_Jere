...
Après l'apprentissage du grec, grâce aux nombreuses équations intégrées au livre, passons au latin ! L'expression *toutes choses étant égales par ailleurs* vient du latin *ceteris paribus* à ne pas confondre avec *c'est terrible Paris en bus* (petite blague formulée par un étudiant ayant suivi le cours *méthodes quantitatives appliquées en études urbaines* il y a quelques années) ! Certains auteurs emploient encore *ceteris paribus*; il est donc possible que vous la retrouviez dans un article scientifique... 
...
Plusieurs options sont possibles. Vous pouvez choisir :
...
Dans ce modèle ci-dessous (tableau \@ref(tab:ModeleInteraction1)), les pourcentages de jeunes de moins de 15 ans et de 65 ans et plus (`Pct_014` et `Pct_65P`) sont associés positivement à la variable dépendante tandis que le pourcentage de personnes à faible revenu (`Pct_FR`) est associé négativement.
...
```{r ModeleInteraction2heat, fig.cap = "Effet marginal de l'interaction entre deux variables continues", echo=FALSE, message=FALSE, warning=FALSE, out.width='95%'}
...
```{r ModeleInteractionBinaireFIG, echo=FALSE, message=FALSE, warning=FALSE, out.width='75%', fig.cap="Graphique de l'effet marginal de l'interaction entre une variable quantitative et qualitative"}
...
Ghisto2 <- ggplot() +
  ...
  stat_function(fun = dnorm,  args = list(mean = mean(resmodele3b),
                                          sd = sd(resmodele3b)), 
                color = "#e63946", size = 1.2, linetype = "dashed")+
  labs(title="Modèle après suppression", y = "densité", "Résidus studentisés")

Gqqplot2 <- qplot(sample = resmodele3b)+
  geom_qq_line(line.p = c(0.25, 0.75), color = "#e63946", size=1.2)+
  labs(title="Modèle après suppression",
     x="Valeurs théoriques", y = "Résidus studentisés")
...
```{r ModeleInteraction2heatex, echo=TRUE, fig.cap="Effet marginal de l'interaction entre deux variables continues", message=FALSE, warning=FALSE, out.width='95%'}
...
```{r ModeleInteractionBinaireeffectex, echo=TRUE, fig.cap="Graphique de l’effet marginal de l’interaction entre une variable quantitative et qualitative", message=FALSE, warning=FALSE, out.width='75%'}

...
# Régression linéaire multiple  {#chap07}
...
* Pour vérifier la normalité des résidus&nbsp;: 
  - `DescTools` avec les fonctions `Skewness` et `Kurtosis` et `JarqueBeraTest` (section \@ref(sect0762)).
* Pour vérifier l'homoscédasticité des résidus&nbsp;: 
  - `lmtest` avec la fonction `bptest` pour le test de Breusch-Pagan (section \@ref(sect07733)).
...
### Hypothèses de la régression linéaire multiple {#sect0722}
...
* **Hypothèse 7**. *Le modèle est bien spécifié*. Un modèle est mal spécifié (construit) quand «&nbsp;une ou plusieurs variables non pertinentes sont incluses dans le modèle&nbsp;» ou «&nbsp;qu’une ou plusieurs variables pertinentes sont exclues du modèle&nbsp;» [@bressoux2010, pp. 138-139]. Concrètement, l'inclusion d'une variable non pertinente ou l'omission d'une variable peut entraîner une mauvaise estimation des effets des variables explicatives du modèle.

Pour connaître les conséquences de la violation de chacune de ces hypothèses, vous pourrez notamment consulter l'excellent ouvrage de Bressoux [-@bressoux2010, pp. 103-110]. Retenez ici que le non-respect de ces hypothèses produit des coefficients de régression biaisés.


## Évaluation de  la qualité d’ajustement du modèle {#sect073}
...
### Mesures de la qualité d'un modèle {#sect0731}
...
### Comparaison des modèles incrémentiels {#sect0732}
...
## Différentes mesures pour les coefficients de régression {#sect074}

La fonction `summary(nom du modèle)` permet d'obtenir les résultats du modèle de régression. D'emblée, signalons que le modèle est globalement significatif (*F*(6,10203)&nbsp;=&nbsp;1123, *p*&nbsp;=&nbsp;0,000) avec un R^2^ de 0,4182 indiquant que les variables indépendantes du modèle expliquent 41,82nbsp;% de la variance du pourcentage de végétation dans les îlots de l'île de Montréal.
...
### Coefficients de régression&nbsp;: évaluer l'effet des variables indépendantes {#sect0741}
...
* en calculant les écarts-types de VD et de VI et en appliquant l'équation \@ref(eq:CoefStand);
...
### Exploration des relations non linéaires  {#sect0751}
...
Sachez qu'il est aussi possible de construire des régressions polynomiales avec plus deux ordres. Par exemple, une régression polynomiale d'ordre 3 comprend une variable dans sa forme originelle, puis mise au carré et au cube. Cela a l'inconvénient d'augmenter corollairement le nombre de coefficients. Nous verrons au chapitre \@ref(chap11) qu'il existe une solution plus élégante et efficace&nbsp;: le recours aux modèles de régressions linéaires généralisés additifs avec des *splines*. Dans le cadre de cette section, nous nous limitons à des régressions polynomiales d'ordre 2.
...
```{r EffetMarginalVariableLogEtNon, echo=TRUE, fig.align='center', fig.cap="Effet marginal de la densité de population", message=FALSE, warning=FALSE, out.width='75%', dev="png", dpi=300}
library(ggpubr)
library(ggplot2)
library(ggeffects)
...
#### Comment construire un modèle de régression avec une variable explicative qualitative polytomique? {#sect07531}
...
* Transformer chaque modalité en variable muette (binaire). Nous avons ainsi quatre nouvelles variables binaires&nbsp;: `X2A`, `X2B`, `X2C` et `X2D`. Par exemple, pour `X2A`, les observations de la modalité *A* se verront affecter la valeur de 1 versus 0 pour les autres observations. La même démarche s'applique à `X2B`, `X2C` et `X2D` (voir tableau \@ref(tab:transfVarQMuettes)).
...
#### Effet marginal d'une variable explicative qualitative polytomique  {#sect07533}
...
Dans ce modèle (tableau \@ref(tab:ModeleInteraction1)), les pourcentages d'enfants de moins de 15 ans et de 65 ans et plus (`Pct_014` et `Pct_65P`) sont associés positivement à la variable dépendante tandis que le pourcentage de personnes à faible revenu (`Pct_FR`) est associé négativement.
...
```{r ModeleInteraction2heat, echo=FALSE, fig.cap="Effet marginal de l'interaction entre deux variables continues", message=FALSE, warning=FALSE, out.width='95%', dev="png", dpi=300}
...
```{r ModeleInteraction2heat2, fig.cap = "Effets marginaux de deux variables continues en cas d'absence d'interaction", echo=FALSE, message=FALSE, warning=FALSE, out.width='75%', dev="png", dpi=300}
...
```{r ModeleInteractionBinaireFIG, echo=FALSE, fig.cap="Graphique de l’effet marginal de l’interaction entre une variable quantitative et qualitative", message=FALSE, warning=FALSE, out.width='75%', fig.align='center', dev="png", dpi=300}
...
```{r VerifierNormaliteResidus, echo=FALSE, fig.align='center', fig.cap="Vérification de la normalité des résidus", message=FALSE, warning=FALSE, out.width='75%'}
library(DescTools)
library(ggplot2)
library(ggpubr)
...
```{r VerifierHomoscedasticiteFig, echo=FALSE, fig.align='center', fig.cap="Distribution des résidus en fonction des valeurs prédites", message=FALSE, warning=FALSE, out.width='75%', dev="png", dpi=300}
residus <-  modele3$residuals
...
```{r GraphCookLev, echo=FALSE, fig.align='center', fig.cap="Repérage graphique les valeurs influentes du modèle", message=FALSE, warning=FALSE, out.width='75%', dev="png", dpi=300}
library(car)
library(ggpubr)
DistanceCook <- cooks.distance(modele3)
...
* **Utiliser un modèle linéaire généralisé (GLM)** permettant d'utiliser une distribution différente correspondant plus à votre jeu de données (chapitre \@ref(chap08)).


## Mise en œuvre dans R {#sect077}

### Fonctions `lm`, `summary()` et `confint()` {#sect0771}
...
À la figure \@ref(fig:figOutputRegression), les résultats de la régression linéaire multiple, obtenus avec la `summary(monModele)`, sont présentés en quatre sections distinctes&nbsp;:
...
```{r figOutputRegression, fig.align='center', echo=FALSE, auto_pdf = TRUE, fig.cap="Différentes parties obtenues avec la fonction summary(Modèle)",  out.width='75%', fig.pos="H"}
...
### Comparaison des modèles {#sect0772}
...
#### Vérification le nombre d'observations {#sect07731}
...
#### Vérification la normalité des résidus {#sect07732}
...
```{r VerifierNormaliteResidusDia, echo=TRUE, fig.align='center', fig.cap="Diagnostic : la normalité des résidus", message=FALSE, warning=FALSE, out.width='75%'}
library(DescTools)
library(stats)
library(ggplot2)
library(ggpubr)
...
#### Évaluation de la linéarité et l'homoscédasticité des résidus {#sect07733}
...
```{r DiaHomoscedasticite, echo=FALSE, fig.align='center', fig.cap="Distribution des résidus en fonction des valeurs prédites", message=FALSE, warning=FALSE, out.width='75%', dev="png", dpi=300}
ggplot() +
        geom_point(aes(x = ypredits, y = residus),
                       color = "#343a40", fill = "#a8dadc",
                       alpha = 0.2, size = 0.8) +
...
#### Vérification la multicolinéarité excessive {#sect07734}
...
#### Répérage des valeurs très influentes du modèle {#sect07735}
...
```{r DiaGraphCookLev, echo=TRUE, fig.align='center', fig.cap="Repérage graphique des valeurs influentes du modèle", message=FALSE, warning=FALSE, out.width='75%', dev="png", dpi=300}
library(car)
library(ggpubr)
DistanceCook <- cooks.distance(modele3)
LeverageValue <- hatvalues(modele3)
...
#### Construction d'un nouveau modèle en supprimant les observations très influentes du modèle {#sect07736}
...
```{r HomosComp, echo=FALSE, fig.align='center', fig.cap="Amélioration de l'homoscédasticité des résidus", message=FALSE, warning=FALSE, out.width='65%', dev="png", dpi=300}
resmodele3  <- residuals(modele3)
resmodele3b <- residuals(modele3b)
ypredits3 <- modele3$fitted.values
ypredits3b <- modele3b$fitted.values
...
```{r ModeleInteraction2heatex, echo=TRUE, fig.align='center', fig.cap="Effet marginal de l'interaction entre deux variables continues", message=FALSE, warning=FALSE, out.width='95%', dev="png", dpi=300}
...
```{r ModeleInteractionBinaireeffectex, echo=TRUE, message=FALSE, warning=FALSE, fig.align='center', fig.cap="Graphique de l’effet marginal de l’interaction entre une variable quantitative et qualitative", dev="png", dpi=300}

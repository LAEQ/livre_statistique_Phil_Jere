...
Ce livre propose des exemples détaillés et appliqués dans R pour chacune des méthodes abordées. Ces exemples se basent sur des jeux de données structurés et mis à disposition avec le livre. Ils sont disponibles dans le [repos github du livre](https://github.com/LAEQ/livre_statistique_Phil_Jere/tree/master/data). 
...
clean_text <- function(string){
  ...
  string <- gsub("\\$", "+DOLLAR+", string, fixed = TRUE,  useBytes = TRUE)
  changes <- list(
     c("`", "", F),
  ...
  if(grepl("$",string, fixed = TRUE, useBytes = TRUE)){
  ...
  new_strings <- sapply(1:length(parts), function(i){
    ...
    if (thisstring != ""){
      ...
      new_strings2 <- sapply(1:length(parts2), function(j){
        ...
        if (j %% 2 ==0){
          thisstring2 <- gsub("^","\\textasciicircum{}",thisstring2, fixed = T,  useBytes = TRUE)
          return(thisstring2)
        }else{
          return(thisstring2)
        }
    ...
    if (i %% 2 ==0){
      ...
      for (ch in changes){
          if(ch[[3]]){
            thisstring <- gsub(ch[[1]],ch[[2]],thisstring, fixed = T,  useBytes = TRUE)
          }else{
            thisstring <- gsub(ch[[1]],ch[[2]],thisstring, perl = T,  useBytes = TRUE)
          }
  ...
  fullstring <- gsub("+DOLLAR+", "\\$" , fullstring, fixed = TRUE,  useBytes = TRUE)
...
show_table <- function(df, col.names = NA, caption = NULL, col.to.resize = NULL, 
  ...
  if (knitr::is_latex_output()){
    ...
    if (length(col.names) > 0){
      if(is.na(col.names)[[1]]==F){
        #col.names <- gsub("\\(\\%\\)","(\\\\%)",col.names)
        col.names <- gsub("\\%","\\\\%",col.names, useBytes = TRUE)
        col.names <- gsub("\\^([^\"]*?)\\^","\\\\textasciicircum{\\1}",col.names,fixed = F, useBytes = TRUE)
      }
    }

    if(is.null(caption)[[1]]==F){
      caption <- gsub("\\*{2}([^\"]*)\\*{2}","\\\\textbf{\\1}", caption, useBytes = TRUE)
      caption <- gsub("\\*([^\"]*?)\\*","\\\\textit{\\1}", caption, useBytes = TRUE)
    }
    if(length(df2) == 1){
      df2 <- df2[[1]]
    }
  ...
  ## dans le cas d'un tableau HTLM
    ...
    if(class(df)[[1]] != "list"){
      ...
      for(col in names(df)){
        if(is.character(df[[col]])){
          df[[col]] <- gsub("\\newline","<br/>",df[[col]], fixed = T, useBytes = TRUE)
        }
      }
    }else {
    ...
    table1 <- knitr::kable(
      ...
      escape = FALSE, ...)
    ## gerer le padding
    table1 <- gsub(pattern = "NApadding", replacement = "padding",x = table1, fixed = T,  useBytes = TRUE)
  }
  return(table1)
}

...
* **Hypothèse 5**. *Homoscédasticité des erreurs (ou absence d'hétéroscédasticité)*. Il y a homoscédasticité lorsqu'il y a une absence de corrélation entre les résidus et les valeurs prédites.
* **Hypothèse 6**. *Le modèle est bien spécifié*. On dira qu'un modèle est mal spécifié (construit) quand « une ou plusieurs variables non pertinentes sont incluses dans le modèle » ou « qu’une ou plusieurs variables pertinentes sont exclues du modèle » [@bressoux2010, p. 138-139]. Concrètement, l'inclusion d'une variable non pertinente ou l'omission d'une variable peut entraîner une mauvaise estimation des effets des variables explicatives du modèle.
...
G1a <- ggplot(fit1a, aes(x, predicted)) +
      geom_point(data = DataFinal, mapping = aes(x=HABHA, y = VegPct), size = 0.2, color = rgb(0.1,0.1,0.1,0.4)) + 
...
Dans ce modèle ci-dessous (tableau \@ref(tab:ModeleInteraction1)), les pourcentages de jeunes de moins de 15 ans et de 65 ans (`Pct_014` et `Pct_65P`) et plus sont associés positivement à la variable dépendante tandis que avec celui des personnes à faible revenu (`Pct_FR`) est associé négativement.

Que se passe-t-il si nous introduisons une variables d'interaction entre `DistCBDkm` et `Pct_FR` (tableau \@ref(tab:ModeleInteraction2))? L’effet du pourcentage de personnes à faible revenu (%) est significatif et négatif lorsqu’elle est mise en interaction avec la distance au centre-ville. Cela indique que plus l'îlot est éloigné du centre-ville, plus `Pct_FR` a un effet négatif sur la couverture végétale (B=−0,011, P<0,001).
...
À nouveau, il est possible de représenter l'effet de cette interaction à l'aide d'un graphique des effets marginaux. Notez cependant que nous devons représenter l'effet simultané de deux variables indépendantes sur notre variables dépendantes, ce que nous pouvons faire avec une carte de chaleur. La figure \@ref(fig:ModeleInteraction2heat) représente donc l'effet moyen de l'interraction sur la prédiction dans le premier panneau,  ainsi que l'intervalle de confiance à 95% de la prédiction dans le deuxième et troisième panneau. On constate ainsi le modèle prédit des valeurs de végétation les plus faibles lorsque le niveau de personnes faible revenu est élevé et que la distance au centre ville est élevée (en haut à droite). En revanche, les valeurs les plus élevés de végétation sont atteintes lorsque la distance au centre ville est élevée et que le pourcentage de personnes à faible revenu est bas (en bas à droite). Il semble donc que l'éloignement au centre-ville soit associé avec une augmentation de la densité végétale, mais que cette augmentation puisse être mitigée par l'augmentation parallèle de la proporition de personnes à faible revenu.
...
#### Variable d'interaction entre une variable continue et une variable dichtomique {#sect05542}
...
* à chaque augmentation d'une unité du pourcentage à faible revenu (`Pct_FR`), la pourcentage de la couverture végétale diminue significativement de −0,444;
...
L'interraction entre une variable qualitative et une variable quantitative peut être représentée par un graphique des effets marginaux. La pente (coefficient) de la variable quantitative varie en fonction des catégories de la variables qualitative.
...
Pour illuster comment vérifier si le modèle respecte les hypothèses de la régression, nous utiliserons le modèle suivant :
...
Dans leur ouvrage classique intitulé *Using Multivariate Statistics*, Barbara Tabachnick et Linda Fidell [-@tabachnick2007, pp. 123-124] proposent deux règles à la louche :

1. $n \geq 50 + 8k$ avec *k* avec $N$ et $k$ étant respectivement les nombre d'observations et de variables indépendantes, pour tester le coefficient de corrélation multiples ($R^2$).
...
* les tests de normalité, particulièrement le test de Shapiro-Wilk 
* les graphiques (histogramme et courbe normale et diagramme quantile-quantile)

Les deux premiers étant parfois très restrictifs, on porte habituellement une attention particulière aux graphiques.


### Linéarité et homoscédasticité des résidus {#sect0564}


### Absence de multicolinéarité excessive {#sect0565}


### Absence de valeurs aberantes {#sect0566}

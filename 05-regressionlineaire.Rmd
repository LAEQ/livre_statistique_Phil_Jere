...
Dans l'exemple précédent, les chercheurs voulaient évaluer l'impact des caractéristiques de l'environnement urbain (variables explicatives) traversés par les cyclistes sur leurs expositions au dioxyde d'azote et au bruit, une fois contrôlés les effets de facteurs reconnus comme ayant un impact significatif sur la concentration de polluants comme les conditions météorologiques et la pollution d’arrière-plan. Autrement dit, si les variables de contrôle n'avaient pas été prise en compte, <span class = "revision_rem">le modèle aurait été imparfait : l'ampleur des effets des caractéristiques de l'environnement urbain sur les expositions des cyclistes aurait alors été biaisée.</span><span class = "revision_prop"> l'étude des variables d'intérêt serait biaisée par les effets de ces facteurs qui n'auraient pas été contrôlés. À titre d'exemple, il serait possible que les zones de circulation limitées à 30 km/h soient concentrées dans les quartiers centraux et denses de Paris. Dans ces quartiers, la pollution d'arrière plan a tendance à être supérieure. Si l'on ne tient pas compte de cette pollution d'arrière plan, on pourrait arriver à la conclusion que les zones 30 sont des milieux dans lesquels les cyclistes sont plus exposés à la pollution atmosphérique.</span>

<span class = "revision_rem">
Cela explique que certains chercheurs vont parfois construire un premier modèle avec uniquement les variables de contrôle, puis un second dans lequel sont ajoutées les variables explicatives.</span>
<span class = "revision_prop">on se repete avec en dessous ici</span>
...
On dit donc de deux modèles qu'ils sont imbriqués si le modèle avec le plus de variables comprends également **toutes** les variables du modèle avec le moins de variable.
</span>
:::
...
<span class = "revision_prop">Est-ce necessaire de mettre les trois formules ?</span>
...
<span class = "revision_prop">Les sections ne se suivent pas bien, mesurer la qualité d'ajustetement devrait aller au même niveau que les hypothèse de la régression non ?</span>
...
<span class = "revision_prop">On pourrait donner plus de contexte sur le modèle : pourquoi ces groupes ? </span>
...
<span class = "revision_prop">Évoquer le principe de parcimonie et du razoir d'Ockham ?</span>
...
<span class = "revision_prop">On se répète non ? Est-ce que l'on parle ici plutôt d'interpréter les coefficients du modèle ?</span>
...
<span class = "revision_prop">
Notez que la constante n'a pas toujours une interprétation pratique. Il est par exemple très invraissemblable de trouver un ilôt peuplé sur l'Île de Montréal dans lequel il n'y aurait aucune personne à faible revenu, faisant partie d'une minorité visible, ayant moins de 14 ans ou plus de 65 ans. De même, il est impossible d'observer un ilôt où l'âge médian serait de 0 année. La constante a donc avant tout un rôle mathématique dans le modèle.
Est-ce que l'on parlerait des cas où la constante s'interprete bien ?
</span>
...
L'erreur type d'un coefficient permet d'évaluer son niveau de précision, <span class = "revision_prop">soit le degré d'incertitude vis à vis du coefficient</span>. Succinctement, elle correspond à l'écart-type de l'estimation (coefficient); elle est ainsi toujours positive. Plus la valeur de l'erreur type est faible, plus l'estimation du coefficient est précise. Notez toutefois qu'il n'est pas judicieux de comparer les erreurs type des coefficients pour des variables exprimées dans des unités de mesure différents. 
...
* avec la fonction `lm.beta` du package **QuantPsyc**. Cette dernière méthode est moins « verbeuse » (deux lignes de codes uniquement), <span class = "revision_prop">mais nécessite de charger un package supplémentaire</span>.


<span class = "revision_prop">Dans une logique d'harmonisation du code, il faudrait qu'on décide comment on présente nos sections de commentaires</span>
...
<span class = "revision_prop">Est-ce qu'un effet plus important se traduit vraiment par un apport plus important au modele ?</span>
...
<span class = "revision_prop">Interprétation littérale de la valeur p : Si en réalité le coefficient pour la population totale était de 0, alors nous aurions eu moins de 5% de chance de collecter un échantillon produisant un résultat au moins aussi extrème. Il s'agit donc d'une preuve par l'absurde</span>
...
<span class = "revision_prop"> Pas sûr pour ce bloc</span>
...
* il permet d'estimer la précision de l'estimation; plus l'intervalle du coefficient est réduit, plus l'estimation de l'effet de la variable indépendante est précise. Inversement, un intervalle large signale que le coefficient est incertain. <span class = "revision_prop">Cette incertitude peut être associée à un trop faible échantillon ou à une forte variabilité du phénomène étudié</span>.

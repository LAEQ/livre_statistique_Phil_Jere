...
* **Construisez un nuage de points de l'effet marginal** de votre variable afin de vérifier si la relation qu'elle partage avec votre VD est plutôt logarithmique que linéaire (voir le graphique ci-dessous).


JÉRÉMY : SVP FAIRE UN NUAGE DE POINTS AVEC y ET HABHA UNE FOIS CONTROLEES LES AUTRES VARIABLES. PUIS AJOUTER LA DROITE DE RÉGRESSION ET CELLE EN LOG SVP.

MODIFIER LE GRAPHIQUE CI-DESOUS
...
Par conséquent, il est légitime de se questionner sur le bien-fondé de conserver ces variables indépendantes additionnelles : `Muni` pour le modèle 5 et `DistCBDkm + DistCBDkm * Pct_014` pour le modèle 6. Deux options sont alors envisageables :

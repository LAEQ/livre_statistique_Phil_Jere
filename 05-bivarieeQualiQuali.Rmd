...
Pour vérifier l'existence d'associations significatives entre les modalités de deux variables qualitatives, il est possible de construire un **tableau de contingence**tableau de contingence (section \@ref(sect051)), puis de réaliser le **test du khi-deux** (section \@ref(sect052)).
:::
...
```{r EncadreAsso, echo=FALSE, message=FALSE, warning=FALSE, auto_pdf = TRUE}
df<-data.frame(
      ...
      Formule = c("$\\chi^2 = \\sum \\frac{(f_{ij}-ft_{ij})^2}{ft_{ij}}$",
                  ...
                  "$Q_{MH}=(N−1)r^2$",
                  "Obtenue itérativement par \\n maximum de vraisemblance",
                  "$\\phi=\\sqrt{\\frac{\\chi^2}{n}}$",
                  "$V=\\sqrt{\\frac{\\chi^2/n}{min(c-1,l-1)}}$"),
      
      Propr = c("Mesure classique du khi-deux calculée à partir des différences entre les fréquences observées et attendues. Valeur de *p* disponible",
...
```{r figVDC, echo=TRUE, fig.align='center', fig.cap="Figure avec la fonction mosaic du package vcd", fig.pos="H", message=FALSE, warning=FALSE, auto_pdf=TRUE}
library(vcd)
...
## Quiz de révision du chapitre {#sect055}

...
Dans le cadre de ce chapitre, nous présenterons les deux principales méthodes permettant d'explorer les associations entre des variables qualitatives, principalement la construction d'un tableau de contingence et le test du khi-deux ($\chi^2$, appelé aussi khi carré).
...
Comparativement aux deux autres groupes, on pourrait supposer que les jeunes se déplacent proportionnellement plus en modes de transport actif (vélo et marche) et en transports en commun. À l'inverse, il est possible que les 45 à 64 ans se déplacent majoritairement en véhicules motorisés.
...
* **La contribution au khi-deux** est égale à la déviation au carré divisée par la fréquence théorique : $\chi_{ij}^2 = (f_{ij}-ft_{ij})^2/ft_{ij}$. Plus sa valeur est forte, plus il y a association entre les deux modalités. La somme des contributions au khi-deux représente le khi-deux total pour l'ensemble du tableau de contingence (ici à 63,54) que nous aborderons dans la section suivante.
...
Pour calculer le Khi-deux entre deux variables qualitatives, on utilise la fonction de base : `chisq.test(x = ..., y = ...)` qui renvoie le nombre de degrés de liberté, les valeurs du Khi-deux et de *p*.
...
AutresMesuresKhi2 <- function(x, y){
  ...
  chi2 <- testChi2$statistic   # Khi-deux
  ...
  # khi-deux de Mantel-Haenszel avec la librarie DescTools
...
```{r figVDC, fig.pos="H", auto_pdf=TRUE, echo=TRUE, message=FALSE, warning=FALSE, fig.align='center', fig.cap='Figure avec la fonction mosaic de la librarie vcd'}

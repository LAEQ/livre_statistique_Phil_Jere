...
S'il est rare de manipuler des matrices et des *arrays*, le *DataFrame* (tableau de données en français) est la structure de données la plus souvent utilisé. Dans cette structure, chaque ligne du tableau représente un individu et chaque colonne représente une caractéristique de ces individus. Ces colonnes ont des noms, ce qui permet facilement d'accéder à leurs valeurs. Créons un *DataFrame* à partir de nos quatres vecteurs et de la fonction `data.frame`.
...
On pourrait également tout mettre en minuscules ou tout en majuscule.
```{r message=FALSE, warning=FALSE}
...
AJOUTER SUBSTR
COUPER À GAUCHE ET TROIS
SUBSTR(1,4)
SUBSTR(lenght(x)-4, lenght(X))

...
Nous avons souligné dans la section sur la Philosophie de ![](images/logos/Rlogo.png) la place centrale jouée par les packages. Voyons donc ensemble comment installer un package. Nous allons prendre l'exemple du package **lubridate**, qui nous permettra plus tard de manipuler des données temporelles.
...
*CRAN* est le répertoire officiel des packages de ![](images/logos/Rlogo.png). Vous pouvez cependant télécharger des packages provenant d'autres sources. Très souvent, les packages sont disponibles sur le site web [GitHub](https://github.com/){target="_blank"} et on peut même y trouver les versions en développement avec des fonctionnalités encore non intégrées dans la version sur *CRAN*.

Reprenons le cas de **lubridate**, sur GitHub, il est disponible à la page [suivante](https://github.com/tidyverse/lubridate){target="_blank"}. Pour l'installer nous devons d'abord installer un autre package appelé **devtools** (depuis *CRAN*).
...
Souvent, vous aurez besoin d'accéder à la documentation d'une fonction spécifique de notre package. Affichons la documentation de la fonction `now` de **lubridate** : 
...
Si vous connaissez le nom d'une fonction, mais ne savez plus dans quel package elle se trouve, vous pouvez lancer une recherche en utilisant un double point d'interrogation : 
...
Il est possible d'enregistrer le résultat d'une expression et de la conserver dans un nouvel objet. On appelle cette opétation déclarer une variable.

```{r}
ma_somme <- 4 + 3
```

Concrêtement, nous venons de demander à ![](images/logos/Rlogo.png) d'enregistrer le résultat de `4 + 3` dans un espace spécifique de notre mémoire vive. Si vous regardez dans votre fenètre Environment, vous verez en effet qu'un objet appelé ma_somme est actuellement en mémoire et a pour valeur 7.

Notez ici que le nom des variables ne peut être composé que de lettres, de chiffres, de points (.) et de tiret bas (_) et doit commencer par une lettre. ![](images/logos/Rlogo.png) est sensible à la case, en d'autre termes, les variables `Ma_somme`, `ma_sommE`, `ma_SOMME`, et `MA_SOMME` indiquent toutes un objet différent. Attention donc aux fautes de frappes. Si vous déclarez une variable en utilisant le nom d'une variable existante, la première est écrasée par la seconde : 
...
Notez tout de suite que la mémoire vive (l'environnement) est vidée lorsque vous fermez ![](images/logos/Rlogo.png). En d'autres termes, ![](images/logos/Rlogo.png) perd complètement la mémoire lorsque vous le fermez. Vous pouvez bien sûr recréer vos objets en relancer les mêmes syntaxes, c'est pourquoi vous devez conserver vos feuilles de codes et ne pas seulement travailler dans la console. La console ne garde aucune trace de votre travail. Pensez donc bien à enregistrer votre code.

Nous verrons plus tard comment sauvegarder des objets et les recharger dans une session ultérieure de ![](images/logos/Rlogo.png). Ce type d'opération est pertinente quand le temps de calcul nécessaire à la production de certains objets est très longue.
...
round(taille)
```
Sans surpise, nous obtenons 176. Pour effectuer leurs opérations, les fonctions ont généralement besoin d'**arguments**. Ici, `taille` est un argument passé à la fonction `round`. Si nous regardons la documentation de `round` avec `help(round)`, nous pouvons constater que cette fonction prends en réalité deux argments : x et digits. Le premier est le nombre que nous souhaitons arrondir et le second le nombre de décimal à conserver. On peut observer dans la documentation que la valeur par défaut de digits est 0, c'est pourquoi `round(taille)` a produit le résultat 176.
...
```
Il est aussi possible que certaines fonctions ne requiert par d'arguments. Par exemple la fonction `now` va nous indiquer quelle heure il est actuellement et n'a besoin d'aucun argument pour le faire : 

```{r}
now()
```

Par contre, si nous essayons de lancer la fonction round sans argument, nous allons obtenir une erreur : 
...
Le message est très clair, `round` a besoin d'au moin 1 argument pour fonctionner. Si au lieu d'un nombre, nous avions donné du texte à la fonction `round`, nous aurions aussi obtenu une erreur : 

```{r eval = FALSE}
round("Hello World")
```
<span class = "error_message">Error in round("Hello World") : 
  argument non numérique pour une fonction mathématique</span>

À nouveau le message est très explicit : nous avons donnée un argument non-numérique à une fonction mathématique. Prenez le temps de toujours lire vos messages d'erreurs, ils vous permettront d'identifier bien des coquilles dans vos codes.

Une fonction essentielle à connaître est la fonction `print`. Elle permet de demander à ![](images/logos/Rlogo.png) d'afficher la valeur d'une variable. Par exemple : 
...
Nous avons vu que les fonctions nous permettent de manipuler des objets. Nous pouvons également effectuer un grand nombre d'opération avec des opérateurs.
...
Les opérateurs relationnels permettent de vérifier des conditions dans ![](images/logos/Rlogo.png). Ils renvoient un booléens, TRUE si la condition est vérifiée et FALSE si ce n'est pas le cas.
...
Prenons le temps pour un petit exemple : 
...
# produit TRUE car la au moins une des deux conditions est juste
a < b |  b < c

```
Notez que l'opérateur ET a la priorité sur l'opérateur OU et que les parenthèses ont la priorité sur tous les opérateurs, ainsi : 
...
```
Notez qu'en arrière plan, les opérateurs sont en réalité des fonctions déguisées. Il est donc possible de définir de nouveau comportements pour les opérateurs. Il est par exemple possible d'additionner ou comparer des objets spéciaux comme des dates, des géométries des graphs, etc.

### Structures de données

Jusqu'ici, nous avons travaillé avec des objets ne comprenant qu'une seule valeur. Lors d'une analyse statistique, nous allons travailler avec des volumes de données bien plus conséquents. Pour stocker plusieurs valeurs, nous allons travailler avec des structures de données que sont les vecteurs, les matrices, les dataframes et les listes. Nous allons présenter ces différents types d'objets et leurs usages.
...
Nous venons ainsi de déclarer 4 nouvelles variables étant chacune un vecteur de longueur 5 (comprenant chacun 5 valeurs). Ces vecteurs représentent, par exemple, les réponses de plusieurs répondants à un questionnaire.
...
Il est possible de combiner des vecteurs pour former des matrices. Une matrice est un tableau en deux dimensions (colonnes et lignes) généralement utilisé pour représenter certaines structures de données comme des réseaux ou des images (pixels), effectuer du calcul matriciel ou plus simplement présenter des matrices de corrélations. Vous aurez rarement à travailler directement avec des matrices, mais il est bon de savoir ce qu'elles sont.

Créons deux matrices à partir de nos précédents vecteurs.
...
```{r fig012, fig.align='center', echo=FALSE, include=identical(knitr:::pandoc_to(), 'html'), auto_pdf = TRUE, fig.cap="Du vecteur à la matrice",  out.width='60%'}
...
S'il est rare de travailler avec des matrices et des arrays, le *DataFrame* (tableau de données en français) est en revanche la structure de données avec laquelle vous travaillerez 95% de votre temps. Dans cette structure, chaque ligne du tableau représente un individu et chaque colonne représente une information pour ces individus. Ces colonnes ont de noms, ce qui permet facilement d'accéder à leurs valeurs. Créons ensemble un DataFrame à partir de nos quatres vecteurs et de la fonction `data.frame`.
...
La figure \@ref(fig:fig014) résume ce passage d'une simple donnée à un DataFrame en passant par un vecteur.

```{r fig014, fig.align='center', echo=FALSE, include=identical(knitr:::pandoc_to(), 'html'), auto_pdf = TRUE, fig.cap="De la donnée au DataFrame",  out.width='60%'}
...
La dernière structure de données que vous voulons vous présenter sont les listes. Une liste ressemble à un vecteur, au sens où elle permet de stocker un ensemble d'objet les uns à la suite des autres. Cependant, une liste peut contenir n'importe quel type d'objet. Vous pouvez ainsi construire des listes de matrices, des listes d'arrays, des listes de listes, des listes mixant des vecteurs, des graphiques, des DataFrames etc.

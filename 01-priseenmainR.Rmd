...
![](images/logos/Rlogo.png) est un langage de programmation. Il vous permet de communiquer avec votre ordinateur pour lui donner des tâches à accomplir. Dans cette section, nous allons couvrir les bases de ce langage. Cette section d'introduction est présente dans tous les manuels sur ![](images/logos/Rlogo.png). Elle peut paraître arride à première vue car nous ne réalisons pas d'analyse à proprement parler. Gardez cependant en tête qu'au moins les deux tiers de votre temps en analyse de données consistera à nettoyer et préparer les données en vue de l'analyse. Bien maîtriser les bases du langage peut donc vous faire gagner un temps précieux. Nous vous recommandons donc de faire une première lecture de ce chapitre pour comprendre quelles manipulations vous pouvez effectuer avec ![](images/logos/Rlogo.png), et éventuellement revenir sur les sections nécessaires lorsque vous en aurez besoin.
...
* Le paramètre *stringsAsFactors* permet d'indiquer à ![](images/logos/Rlogo.png) que les colonnes comportant du texte doivent être chargées comme des vecteurs de type texte et nom de type facteur.
...
#### Combiner des *DataFrame*

Terminons cette section avec la combinaison de *DataFrames*. Il existe deux façons de combiner des *DataFrames* : par ajout ou par jointure.

##### Combiner des *DataFrame* par ajout
...
```{r fig016, fig.align='center', echo=FALSE, include=identical(knitr:::pandoc_to(), 'html'), auto_pdf = TRUE, fig.cap="Concaténation de DataFrames",  out.width='30%'}
knitr::include_graphics('images/introduction/rbind_cbind.png', dpi = NA)
```

Pour que `cbind` fonctionne, il faut que les deux *DataFrames* qui lui sont passés aient le même nombre de lignes. Pour rbind, les deux *DataFrames* doivent avoir le même nombre de colonnes.

Prenons à nouveau comme exemple le jeu de données iris. Nous allons commencer par le séparer en trois sous jeu de données comprenant chacun une espère d'iris. Nous allons ensuite en recombiner deux avec la fonctions `rbind`.
...
Nous pourrions aussi extraire dans deux *DataFrames* les colonnes comprenant le mot Length et le mot Width, puis les recombiner.

```{r message=FALSE, warning=FALSE}
iris1 <- iris[names(iris)[grepl("Length",names(iris), fixed = TRUE)]]
iris2 <- iris[names(iris)[grepl("Width",names(iris), fixed = TRUE)]]

iris_comb <- cbind(iris1,iris2)
names(iris_comb)
```
##### Joindre des *DataFrame*

Une jointure est une opération un peu plus complexe qu'un simple ajout. L'idée est d'associer des informations de plusieurs *DataFrames* en utilisant une colonne conjointe (appelée une clef) aux deux jeux de données. On distingue plusieurs type de jointures : 
...
Ces trois jointures sont présentées à la figure \@ref(fig:fig016). Dans les trois cas, la colonne commune se nomme *id*.
...
Vous noterez que les deux dernière jointures peuvent produire des valeurs manquantes. Pour effectuer ces opérations, il est possible d'utiliser la fonction `merge`. Prenons un exemple simple à partir d'un petit jeu de données.
...
Nous avons donc deux *DataFrames*, le premier décrivant des livres et le second des auteurs. Effectuons une première jointure interne afin de savoir pour chaque livre la nationnalité de son auteurs et si ce dernier est à la retraite.

```{r message=FALSE, warning=FALSE}
df1 <- merge(books, authors, #les deux DataFrames 
             by.x = "aut", by.y = 'name', #les noms des colonnes de jointures
             all.x = FALSE, all.y = FALSE)

print(df1)
```
Cette jointure est interne car les deux paramètres *all.x* et *all.y* ont pour valeur `FALSE`. Ainsi, nous indiquons à la fonction que noue ne souhaitons ni garder tous les éléments du premier DataFrame, ni tous les éléments du second, mais uniquement les éléments présents dans les deux. Vous noterez ainsi que le livre "R for Data Science" n'est pas présent dans le jeu de données final car son auteur "Wickham" ne fait pas partie du *DataFrame* authors. De même, l'auteur "Apparicio" n'apparaît pas dans la jointure car aucun livre dans le *DataFrame* books n'a été écrit par cet auteur.

Pour conserver tous les livres, nous pouvons effectuer une joiture à gauche en renseignant `all.x = TRUE`. Nous allons ainsi forcer la fonction à garder tous les livres et mettre des valeurs vides aux informations manquantes des auteurs.

```{r message=FALSE, warning=FALSE}
df2 <- merge(books, authors, #les deux DataFrames 
             by.x = "aut", by.y = 'name', #les noms des colonnes de jointures
...
```{r message=FALSE, warning=FALSE}
df3 <- merge(books, authors, #les deux DataFrames 
             by.x = "aut", by.y = 'name', #les noms des colonnes de jointures
...
Voilà qui conclue ce chapitre sur les bases du langage ![](images/logos/Rlogo.png). Vous avez maintenant les bases nécessaires pour commencer à travailler. N'hésitez pas à revenir sur les différentes sous-sections au besoin !

Pour aller plus loin dans l'apprentissage du langage, vous pouvez également vous plonger dans la section R AVANCÉ. Cependant, nous vous recommandons de faire vos premiers pas avec cette base avant de vous lancer dans cette partie davantage orientée programmation. Voici également quelques ressouces pertinentes qui pourraient vous être utiles : 
...
df <- data.frame(
        ...
        Description = c("Un recueil de nombreux blogs parlant de R, parfait pour être tenu au courant des nouveautés et faire des découvertes", "Les derniers packages publiés sur CRAN, permet de garder un oeil sur les nouvelles fonctionnalités de ses packages préférés", "Une excellente ressource en français pour en apprendre plus sur le Tidyverse", "Une chaîne youtube pour revoir les bases de R en vidéo", "Des feuilles de triches résumant les fonctionnalités de nombreux packages"))

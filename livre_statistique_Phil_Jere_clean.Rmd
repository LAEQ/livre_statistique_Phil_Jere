...
# Régressions régression multiniveaux  {#chap08}
...
## Les modèles multiniveaux : deux intérêts majeurs {#sect081}
...
### La répartition de la variance entre les différents niveaux {#sect0811}
...
### L'estimation des coefficients aux différents niveaux {#sect0812}
...
## Les différents types de modèles multiniveaux {#sect082}

### Description du jeu de données utilisé {#sect0821} 
...
### Démarche classique pour les modèles multiniveaux {#sect0822} 
...
#### Modèle avec les variables indépendantes du niveau 1 {#sect08222}

Dans ce second modèle, on introduit uniquement les variables explicatives au niveau 1. Par conséquent, ce modèle est tout simplement un modèle à effets mixtes (GLMM) avec des constantes aléatoires largement décrit à la section \@ref(sect07121)). Si vous comparez l'équation du modèle vide (équation \@ref(eq:multiniveauIcc)) avec l'équation de ce modèle (équation \@ref(eq:multiniveauModele2)), vous constaterez que le paramètre $\beta_1 x_1$ a été ajouté. Il correspond au coefficient pour la variable indépendante $X_1$ mesurée au niveau 1 (effet fixe). On pourrait alors ajouter d'autres paramètres pour les autres variables indépendantes du modèle, soit $\beta_1 x_1 + \beta_2 x_2+ \ldots + \beta_k x_k$ ($k$ étant le nombre de variables explicatives mesurées au niveau 1, effets fixes).
...
* plus la valeur de l'AIC est faible, mieux le modèle est ajusté. En comparant les valeurs d'AIC pour le modèle vide et celui avec les variables explicatives du niveau 1 (80305 *versus* 78785), on constate sans surprise que ce dernier modèle est plus performant.

* le R^2^ marginal indique la proportion de la variance expliquée si uniquement les effets fixes avaient été pris en compte (ici 0,129). Quant au R^2^ conditionnel, il indique la proportion variance expliquée à la fois par les effets fixes et aléatoires (ici, 0,268). L'écart important entre les deux R^2^ signale que les secteurs de recensements (effets aléatoires, niveau 2) jouent un rôle important dans le modèle.
...
**Pourquoi la pratique du centrage en analyses multiniveaux est si courante?**
...
#### Modèle complet avec les variables indépendantes aux niveaux 1 et 2 {#sect08223}

Le troisième type de modèle consiste à introduire à la fois les variables explicatives mesurées au niveau 1 et au niveau 2 (équation \@ref(eq:multiniveauModele3)). Il est alors communément appelé le modèle complet. Les variables explicatives du niveau 2 sont aussi des effets fixes.
...
#### Modèle avec une interaction entre deux niveaux {#sect08224}
...
## Conditions d'application des régression multiniveau {#sect083}
...
1. l'absence de multicollinéarité excessive
2. la normalité des résidus
3. l'absence d'observations trop influentes dans le modèle.

**Combien de groupes au niveau 2?** Dans la section \@ref(sect073) Nous avons vu que dans un modèle GLMM certains auteurs, notamment Gelman et Hill 
 [-@gelman2006data], préconisent un minimum de cinq groupes dans un modèle à effets mixtes.
...
### Le modèle avec les variables indépendantes du niveau 1 {#sect08412}
...
### Le modèle avec les variables indépendantes aux niveau 1 et 2 {#sect08413}
...
# MODÈLE 3 : modèle complet avec les prédicteurs au niveau 1 et 2
# ------------------------------------------------------
Modele3 <- lmer(PCTArb ~
                  # Variables indépendantes au niveau 1
                   Width.c+Length.c+AgeMed.c+AgeMed2.c+ResiPCT.c+DuTriPct.c+NoLog.c+Setback.c+
...
### Le modèle avec les variables indépendantes aux niveau 1 et 2 {#sect08414}
...
### Comparaison des quatre modèles {#sect08415}

Pour comparer les modèles, on utilise habituellement les statistiques d'ajustement du modèle vu plus haut, soit le maximum de vraissemblance (*−2 Log-likelihood*), l'AIC, l'ICC et les R^2^ marginal et conditionnel.
...
<!--chapter:end:08-Multiniveau.Rmd-->
...
<!--chapter:end:10-AnalysesFactorielles.Rmd-->
...
<!--chapter:end:11-MethodeClassification.Rmd-->
